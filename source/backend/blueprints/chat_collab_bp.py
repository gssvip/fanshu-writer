...
· 段落句式：段落（1–2句成段）可用于冲突/打架/对峙场景的爆发式密集节奏，但整章段数必须遵守「禁令5段数硬上限」（2400字→≤130段），段均长≥18字；超过=漫画分镜脚本化→必须合并相邻2-4句同场景/同镜头/同POV的叙述/动作/对白为一段。主力段长区间=11-35字。
...
· 【段落合并必过】相邻 3 句同 POV/同场景/同镜头有没有合并？连续 ≥4 句一句话独立段有没有合并相邻 2-3 句？段均长≥18字、2400字→段数 ≤130？有没有违反铁律A——为求短段把一句话硬剁成几截（如"他不说话。只是盯着对面那人。""刀光一闪。血溅了半尺。"这种同镜头动作残切=碎句，必须合并成语义完整的单句/复句，不得留半句语义残缺的碎片段）？
