...
2. 扫段尾 → 段尾若是总结/升华/拔高，改成具体动作/画面/后果，或直接删
3. 扫对白 → 角色替作者解释？心理/背景/设定完整说出？→ 拆、删、打断
4. 扫句式 → 连续三句同结构？连续三段同长度？排比三连？→ 打散其中一处
5. 扫路标 → 删不必要"然而/同时/此外/更重要的是/总而言之"
6. 自检读一遍 → 确认未新增内容、未换风格、只做减法

【五、网文创作硬约束（与人味注入一并执行）】
1. 环境描写不超过15%，重点刻画人物动作、微表情、矛盾心理；人物全员有瑕疵、会纠结、口是心非，不许完美人设，禁止OOC。
2. 结尾停在动态动作或悬念上，禁止抒情总结升华；对话有潜台词，不说大道理；所有出场人物有名有姓；章节无缝衔接不跳时间。
3. 没有纯反派，所有角色行为都有合理动机；爽点靠信息差和布局，不开上帝视角；感情线绑定主线、无工业糖精。
4. 绝对不能出现：顿悟式成长、大段景物抒情、人物语气同质化、行为逻辑割裂。
5. 人味注入：加入不完美细节（结巴/重复/打断）、感官碎片、口语化表达，删除冗余形容词。
