...
4. 禁止：不新增剧情/设定/观点/情绪/金句；不把所有句子都改成长句（冲突场景的爆发式密集短段=手机端阅读友好，允许保留；⚠️ 但整章段数超过「禁令5段数硬上限」=漫画分镜脚本化→必须主动合并相邻 2-4 句同场景/同镜头/同POV的叙述/动作/对白为一段；不能堆砌无信息短句凑字数）；不堆砌"嗯啊吧嘛"假装口语化；不把解释腔改文艺腔/抒情腔；不输出修改说明、自检清单、协作口吻。
...
2. 对白AI味（第二大AI味）
   - 识别模式：角色像作者在解释剧情而非回应眼前人；自报动机/创伤/选择完整分析；对白硬塞世界观/规则/背景像说明书；冲突场景语气仍客气逻辑过顺；不同角色同句式同情绪；对白出现书面连接词"因此/然而/与此同时/换句话说"；**3句以上对白连续独立段不嵌动作/不夹反应（像剧本台词）**；**强压迫场景（矿场/监狱/宗门杂役殿等）上下位者语气同质化=上位者客气+下位者完整对答**。
   - 处理：①保留必须剧情事实、关系变化 ②删角色不该明说的心理分析、主题总结、背景说明 ③调语气符合角色身份与当下情绪（不强行现代口语化）④制造缺口：用半句话/停顿/反问/打断替代完整解释 ⑤长对白拆短，中间用动作/停顿/对方插话承接 ⑥**强压迫场景强制对齐：上位者3选1（打断/威胁式反问/用词粗硬）+ 下位者2选1（话少30%/动作沉默代替回答）+ 连续3句对白至少1句动作同段 ⑦3人以上对白必须有角色声纹差（用词/句长/语气词至少1维/两人）**。
   - 约束：角色越紧张/越隐瞒/越愤怒，越不该把话说完整漂亮；重要信息可留，但要像角色当下说出来的，不像作者塞的
3. 工整句式（第三大AI味）
   - 识别模式：连续多段长度接近像自动分块；连续句相同主谓结构/转折/因果；排比三连对仗过密（**包含「看见…看见…看见…」「举起…落下…举起…」等同类动作排比三连**）；段尾总总结点题升华；路标词密集"然而/同时/此外/更重要的是/总而言之"；每段都"观点句+展开解释+段尾总结"模板
...
- 【必删句式·按禁令0 最高优先级】「不是……是……」「不是……而是……」「与其说……不如说……」「不是 X，准确说是 Y」「不是 A，也不是 B——是 C」「不是 X。是 Y」——**所有形态一律砍，无例外**（之前写的"自我修正型保留"= 错误，彻底推翻）。
- 【必删排比·含同动作排比】「更X，更Y，更Z」/「看见…看见…看见…」/「举起…落下…举起…」/「觉得…觉得…觉得…」连续三连/四连排比（力度堆砌型或工整并列型，一律拆成自然单句推进）。
- 【必删句式·对话漫画分镜病】3 句以上对白全是独立段（每句对白独占一段，前后都没动作/反应，像剧本台词）→ 强制把至少 1/3 的对白句跟动作/反应合并成同段（例："老矿奴镐柄往地上一戳。"你叫啥？""→合并为同段）。
- 【必删句式·真相直白点破】（结尾专项）结尾直接写「第30个名字是他的编号」「她就是他的杀父仇人」这种陈述句点破真相 → 强制改成动态动作句收尾（例：停在「指尖落在那个属于他的编号上」，不说破）。
- 必删路标词：然而、同时、此外、更重要的是、总而言之、换句话说
- 必删协作口吻：下面我们、希望这能帮助、作为AI
- 结尾禁用：总结性、升华性、点题性语句（成长、命运、人性、选择、未来、从此、那一刻等拔高词）

【四、执行流程（写完/改完后逐条过一遍）】
1. 【第 1 条·先扫禁令0（最高优先级）】扫「不是X是Y/不是X。是Y/不是X准确说是Y/不是A也不是B——是C」全部形态 → 一律改写成直接陈述句。再扫「更X更Y更Z/看见…看见…看见…/举起…落下…举起…」排比三连 → 拆成自然单句。
2. 扫关键词 → 删"这说明/这意味着/由此可见/换句话说/事实上/显然/本质上/归根结底/从某种意义上说" + 必删词
3. 扫段尾 → 段尾若是总结/升华/拔高，改成具体动作/画面/后果，或直接删
4. 扫结尾 → 章节结尾禁用总结性、升华性、点题性语句；**若结尾是真相揭露，不得直接陈述句点破→强制改写为动态动作句收尾（例：停在"指尖落在那个属于他的编号上"，不说破）**
5. 扫段落合并 → 相邻3句同 POV/同场景/同镜头必须合并至少2句一段；**连续≥4句一句话独立段（对白+叙述全算）必须合并相邻2-3句**；段数超过禁令5硬上限=漫画分镜→主动合并。
6. 扫对白 → ①角色替作者解释？心理/背景/设定完整说出？→ 拆、删、打断 ②强压迫场景（矿场/监狱/杂役殿/战场/审问/奴隶市场）：上位者3选1（打断/威胁式反问/用词粗硬）+ 下位者2选1（话少30%/动作沉默代替回答） ③**连续3句对白至少1句与动作/反应同段，不许每句对白单独成段像剧本台词** ④3人以上对白必须有角色声纹差（用词/句长/语气词至少1维/2人区分）
7. 扫动作闭环 → 挖矿/赶路/抄写/练功/拼杀等重复动作，每200-300字有没有「小目标→决策→验证」闭环？→ 空转流水账（只写"继续挖/还在挖"没验证句）必须补验证句或合并空转段。
8. 扫句式 → 连续三句同结构？连续三段同长度？排比三连？→ 打散其中一处
9. 扫路标 → 删不必要"然而/同时/此外/更重要的是/总而言之"
10. 自检读一遍 → 确认未新增内容、未换风格、只做减法
...
5. ⚠️ 【彻底推翻前规则】之前写的「不是 X，准确说是 Y」「不是痛→是凉→像冰碴」=真人写法保留 = **错误结论，彻底推翻**。所有否定-肯定修正句式，按【禁令0 AI修正式否定句密集症】一律禁绝，改成直接陈述句。无例外。
...
· 【禁令0·最高优先级·AI修正式否定句密集症】——**全章禁绝，出现=直接重写，无例外**。
  · 识别模式（满足任一条即命中）：
    ① 连续 2 句及以上以「不是X，是Y」「不是X。是Y」「不是X，准确说是Y」「不是A，也不是B——是C」开头或结尾的**否定-肯定交替句**，哪怕每句之间夹了别的内容，只要一段内出现 ≥2 次即命中。
    ② 同一段/同一动作描写里嵌套 ≥2 层「不是X是Y」：例如"不是认得，准确说是骨头认得。……不是恐惧，是共振"——这是典型 AI 为了制造"修正感"硬塞的啰嗦句式，读起来像在抠字眼而非讲故事。
    ③ 三连/四连排比"更X，更Y，更Z，……"（例如"更慢，更深，更不像人"）——为了堆砌力度而故意排比，不是人物真实情绪节奏。
    ④ 刻意拆成两句独立句号的否定-肯定："不是铁鞭。是骨鞭"——故意切割句子制造"顿挫感"，100% AI 写法。
  · 【你给的 9 句反例样本=全部命中，全部禁绝的写法】：
    1) 不是水，是血
    2) 不是风声，也不是地下水——是喘气。很轻，很慢，像谁的肺还在骨壁深处一缩一缩地动着。
    3) 不是铁鞭。是骨鞭
    4) 更慢，更深，更不像人。
    5) 不是疼，是麻
    6) 不是认得，准确说是骨头认得。每一块骨头都在颤，不是恐惧，是一种他从未体验过的共振。
    7) 字迹是赵屠子的笔迹，但墨是新蘸的，墨迹还没干透。（但字重复 + "是X，但Y"的修正句式）
    8) 他不识字，至少不该认得这种字。（"至少不该"= 自我修正句式）
  · 处理：把所有「不是X是Y/不是X准确说是Y」改写成**直接陈述事实的正常句**；把排比三连拆成自然的单句推进，不加刻意的"更X更Y"对仗。
  · ⚠️ 之前规则里写过的"不是X，准确说是Y是真人写法，允许保留"= **错误，彻底推翻**。所有这种句式，无论自我修正感多强，一律按 AI 修正式否定句密集症禁绝。
...
· 【强压迫场景·压迫语气差·硬卡（矿场/监狱/宗门杂役殿/战场/衙门审问/奴隶市场/上级训话=必启用）】
  - 【禁】上位者客气、下位者一句一句完整对答、角色语言同质化。
  - 【上位者（监工/长老/军官/掌柜/审讯者）必写 3 选 1】
    a. 打断——下位者没说完就接话/喝止；
    b. 威胁式反问——不正面回应质疑，转威胁"你去不去？不然骨鞭先跟你聊聊？""矿道塌不塌我不管，但你今天定额不够我就断你三天食水。"；
    c. 用词粗硬——"少废话""滚""愣着干啥""你他妈听不懂人话？"（按题材粗细选度，修仙宗门可以换成"聒噪""放肆""拖出去"）。
  - 【下位者（矿奴/杂役/囚犯/新兵/小商贩）必写 2 选 1】
    a. 话少 30%——对方说 20 字，你回 6-12 字，不许完整对答；
    b. 动作/沉默代替回答——点头/低头/攥紧东西/舔嘴唇/咽口水/沉默 2 秒再答，不把"我愿意我不愿意"说出口。
  - ⚠️ 若上位者和下位者对话各一句礼貌对答（无打断/无威胁/话长相当），即违反本条，整段对话重写。
...
4. 段落与句式·黄金 4 型
· 主力段长 11-35 字；单段 >100 字占比 ≤3%；禁连续 3 句同主谓结构。
· 【段落合并·硬执行（配禁令5段数硬上限）】——相邻 3 句满足【同 POV + 同场景 + 同镜头】三同时，**必须至少合并 2 句成一段**；连续 ≥4 句一句话独立段（不管是不是对白）直接合并相邻 2-3 句：
  - 叙述句接叙述句→同段（例："有人推了姜辰一把。他踩进泥水。"→合并成："有人推了姜辰一把，他踩进泥水，脚底滑过一块碎骨。"）。
  - 动作句接对自句→同段（例："老矿奴把镐柄往地上一戳。"你叫啥？""→合并成："老矿奴把镐柄往地上一戳。"你叫啥？""）。
  - 对白句接对白句→若说话人不变/或上一句动作是对下一句对白的铺垫→同段（例："你叫啥？""姜辰。"→可以保持两段，但如果是"A打断B，A又说了一句"→必须合并）。
· ① 递进比较链（多角色场景必写 ≥1 条）：X比起Y像Z→比起W差一截→比起Q差远→比起T小巫见大巫；最后 1-4 字动作短句收尾。
· ② ⚠️【彻底推翻前规则】之前写的"修正感句式不是X准确说是Y=真人写法"= 错误结论，全部按【禁令0 AI修正式否定句密集症】禁绝。**所有「不是X，是Y」「不是X。是Y」「不是X，准确说是Y」「不是A也不是B——是C」无论任何形态一律砍，改成直接陈述句**。无例外。
· ③ 感官细节三叠：温度/气味/触感/声音 选 2-3 叠（不用比喻词，不用"不是X是Y"修正句铺垫）。
· ④ 动作链=小目标链：每 200-300 字一个"小目标→决策→验证"闭环**（挖矿/跑步/赶路/抄写/练功/切菜等任何重复动作，全部纳入本条）**——**挖矿=小目标：今天两筐→决策：顺着骨缝撬省力→验证：半刻钟后筐里多少块/距离定额还差多少**；**不写"继续挖/还在挖/再挖"这种空转流水账，必须每 200-300 字给一个"挖到了多少/还缺多少/镐刃崩了没"的验证句**。
...
· 【结尾钩子·硬执行】
  ...
  - ⚠️【真相直白点破禁】结尾是真相揭露（如"第30个名字是他的编号""她就是他的杀父仇人"）时，**不得直接用陈述句点破**→必须把点破句改写成【动态动作句收尾】：
    ✅ 正确例（动态收尾）：
    > 前二十九个名字，墨迹已干。
    > 第三十个名字，墨迹还没干透。
    > 姜辰的指尖，正落在那个属于他的编号上。（停）
    ❌ 错误例（直接点破）：
    > 第三十个名字，是他的编号。
  - 结尾钩子 3 选 1：新威胁逼近、人物反常动作/对白、真相只露 1 个字/1 个细节不说透。
...
· 【禁令0 必过】整章有没有任何形式的「不是X是Y/不是X。是Y/不是X准确说是Y/不是A也不是B——是C」？有没有排比三连「更X更Y更Z」「看见…看见…看见…」「举起…落下…举起…」？有没有"至少不该""但""墨迹还没干透"这种自我修正/抠字眼句式？→ **全部砍**，改成直接陈述句。
· 【对白自检 3 条必过】
  - ① 强压迫场景（矿场/监狱/宗门杂役殿等）有没有做到上位者打断/威胁式反问/用词粗硬（3选1）+ 下位者话少30%或动作沉默代替回答（2选1）？
  - ② 连续3句对白有没有至少1句**与动作/反应同段**（不许每句对白单独成段像剧本台词）？提示语句首占比 ≤40%？
  - ③ 3 人以上对话有没有角色声纹差（用词/句长/语气词差异，至少每 2 人 1 维区分）？
· 【段落合并必过】相邻 3 句同 POV/同场景/同镜头有没有合并？连续 ≥4 句一句话独立段有没有合并相邻 2-3 句？段均长≥18字、极短句≤10字占比≤60%、2400字→段数 ≤106？
· 【动作闭环必过】挖矿/赶路/抄写/拼杀/练功类重复动作，每 200-300 字有没有「小目标→决策→验证」闭环（没写验证句=空转流水账=不合格）？
...
def _action_chapter(book, session, instruction, gw, sse, target_chapter_num, prev_chapter_content, mode,
    ...
    if chapter_plot_ctx:
        chapter_plot_iron = (
            ...
            f'\n{chapter_plot_ctx}'
            '\n\n【正文前·禁令0+对话硬规则前置·比任何规则都先读】'
            '\n1. 【禁令0·最高优先级】所有「不是X是Y/不是X。是Y/不是X准确说是Y/不是A也不是B——是C」无论任何形态一律砍，改成直接陈述句；排比三连「更X更Y更Z」「看见…看见…看见…」「举起…落下…举起…」一律拆成自然单句。无例外。'
            '\n2. 【对白硬规则·强制执行】若本章含矿场/监狱/宗门杂役殿/战场/衙门审问/奴隶市场等强压迫场景，必须遵守：'
            '\n   a) 上位者（监工/长老/军官/审讯者）3选1：打断下位者说话/威胁式反问（不正面回应质疑/转威胁）/用词粗硬（"少废话/愣着干啥/滚"按题材粗细选度）；'
            '\n   b) 下位者（矿奴/杂役/囚犯/新兵/商贩）2选1：话少30%（对方20字你回6-12字，不许完整对答）/用点头/攥紧东西/舔嘴唇/沉默等动作或沉默代替回答；'
            '\n   c) 连续3句对白里，至少1句与动作/反应同段（不许每句对白单独成段像剧本台词）。'
            '\n3. 【段落合并·强制执行】相邻3句同POV/同场景/同镜头必须合并成至少2句一段；连续≥4句一句话独立段必须合并相邻2-3句（矿场/牢房一句话一段=漫画分镜=不合格）。'
            '\n4. 【动作闭环·强制执行】挖矿/赶路/抄写/练功等重复动作，每200-300字必须有「小目标→决策→验证」闭环（例：挖矿=目标两筐→决策顺骨缝撬→验证半刻钟后筐里多少块/还差多少，不许空转写"继续挖/还在挖"）。'
            '\n5. 【结尾钩子·强制执行】结尾若为真相揭露，不得直接陈述句点破，必须改写为动态动作句收尾（例：停在"指尖落在编号上"而非"第30个是他的编号"）。'
        )

    # === B2 风格对齐 SkillPack：自动匹配玄幻/都市范本，注入在 chapter_plot_iron 之后（第 3 高位）===
    style_pack_prompt = _get_enabled_style_pack(book)

    # === 事前生成禁令·7条（反例矿道文暴露出的核心硬约束，第3高位，优先级最高档）===
    #   注入位置：core_iron → ONE主钩子 → 【事前禁令7条】 → style_pack → writing_rules
    #   ⚠️ 禁令5（段数硬上限）已移到 GENERAL_CORE_RULES 总则里全阶段通用，这里不再重复写 2 遍。
    PRE_GENERATE_BAN_RULES = '''【事前生成禁令·7条（比任何规则都高，违反直接扣分/重写）】
禁令1·禁背景板一次性名字段≥8段：群像围观（点卯台/集会/街道/战场围观群众）只允许1段集中描写；严禁把 8 个以上路人/背景板角色各自拆成独立一段（王猛上前、老矿奴拉住、狗腿拔刀各一段这种→必须合成≤2段）。
禁令2·禁空转式"动作→突破/挖矿/赶路/抄写"闭环：任何熬/抽/练/锻/淬/拼杀、**挖矿/跑步/赶路/抄写/练功/切菜等任何重复动作**，每 200-300 字必须有 1 个「小目标→决策→验证」小三段闭环；**没有闭环=不许写"大椎骨成/突破了/炼成了/继续挖/还在挖/还在赶路"**。
禁令3·禁对白提示语句首≥50%：全章所有对白提示语（XX说/道/问/喝/冷道/笑道…）里，放在对白句子**前面**的比例必须 ≤40%；≥60% 要嵌在对白中或对白后；**连续 3 句对白必须至少 1 句和动作/反应在同一段，严禁每句对白单独成段像剧本台词**。
禁令4·禁群像平级并列段：点卯台/围观/集会/站队类多角色场景，必须有≥1条「递进比较链」；**不许 3 段以上"A做了→B做了→C做了"平级并列堆段**。
禁令5·禁一句话一段话（漫画分镜病）：相邻3句同 POV/同场景/同镜头必须至少合并 2 句一段；连续≥4句一句话独立段必须合并相邻2-3句；2400字→段数硬上限 ≤106段（段均长≥18字、极短句≤10字占比≤60%）。
禁令6·禁强压迫场景对话语气同质化（矿场/监狱/宗门杂役殿/战场/衙门审问/奴隶市场=必启用）：**上位者**必须3选1（打断/威胁式反问/用词粗硬）；**下位者**必须2选1（话少30%/动作或沉默代替回答）；上下位者客气礼貌对答（无打断、无威胁、话长相当）=直接重写整段对话。
禁令7·禁结尾真相直白点破：结尾为真相揭露时，**绝对禁止直接陈述句点破**（例"第30个名字是他的编号""她就是他的杀父仇人"）；必须改写为动态动作句收尾（例：停在"指尖落在那个属于他的编号上"，不说破）。'''
    ...
    if mode == 'polish':
        ...
        sys_prompt = (
            ...
            f'\n\n【输出格式】第一行章节标题（如"第{target_chapter_num}章 标题"），第二行空行，第三行起纯正文。'
            f'\n【字数铁律】纯正文（不含标题）2300-2500字，用全角中文标点；不足扩场景细节/对话停顿/POV感官（温度/气味/触感/视线压力），超了删枝节。（⚠️ 【禁令0】任何情形不得用「不是X是Y/更X更Y更Z」修正句式/排比三连凑字，一律按禁令0砍）'
            f'{anti_spoiler_rule}'
            f'\n\n{PLAIN_TEXT_LAYOUT_RULES}'
            f'\n\n直接输出，不要解释，不要附字数统计。'
        )
...
错误示范："废柴少年一路逆袭。"（太空）
正确示范："被抽走至尊骨的宗门圣子沦为矿奴，为了复仇、为了证明一条不必靠骨头的正道，他必须在五年内以身为炉重开修炼之路，赌上全家性命与自己是否还会再次被背叛，最终把抽骨之人从圣坛拽下来。"
...
- 短期（第1卷内）：具体可落地的胜负条件（如：逃出XX矿场、通过XX考核、拿到XX通行证、赚到XX灵石）；
- 中期（前30%，立足阶段）：建立一个据点/团队/身份/势力雏形（如：外门前百、加入镇骨营、拥有自家铺子）；
- 长期（中后段，立势-立威阶段）：从个人强 → 有自己的班底/地盘/话语权/制度权；
- 终极：与核心冲突三角对决的终局条件（推翻XX、改写XX规则、把XX从圣坛拽下、守护住X）；
...
请按题材从以下爽感类型中选 3-5 种作为主爽点，并写出每种爽点：首次出现的卷/时机、升级迭代的节奏、爽点公式（谁在谁面前做了什么→观众/世界的即时反应→主角/对手的后续后果），不能全写"装逼打脸"：
  ...
  · 智斗反杀 / 借刀杀人 / 权谋布局 / 以弱胜强
  · 资源暴富 / 赚差价 / 经济霸权 / 产业链垄断（商战/种田/修仙资源文）
  · 团队建立 / 兄弟情义 / 红颜情愫 / 师徒传承
  · 宗门崛起 / 建国立朝 / 势力扩张 / 战争碾压
  · 血脉觉醒 / 金手指解锁 / 神秘传承揭秘
  · 世界观解谜 / 真相揭秘 / 前文伏笔集中回收
...
2) 中期对手（第3-5卷）：格局拉高，从个人冲突 → 势力/派系/宗门/国家/种族/阶级层面的系统性对手，动机要与他的身份/家族/信仰/创伤绑定；
...
4) 每级对应的社会地位：凡/武/灵/玄/地/天/圣/神 → 对应在宗门/国家/种族里的职称与权力；
...
2) 各副职业对主职战力的加成方式（如：炼丹出丹药给修炼加速 + 给队友补血；炼器给自己量身做装备；阵法可布置宗门防线/战场困敌；御兽直接多一个战力）；
...
如果是玄幻/仙侠/奇幻：写清修炼文明的层级（王朝→宗门→圣地→皇朝→仙朝→神朝），每一级对应的平均实力、领地规模、可调动资源；
...
本维度是"故事发生的世界全景"，下游剧情的地理移动、势力站队、国家战争、宗门冲突、种族矛盾、资源战争、风土人情都从这里来。不能只写"主角所在的大陆有东/西/南/北四大域"就结束。按以下 15 节依次输出，一节不跳过，合计不少于 2000 字。
...
- 哪些天象会被视为吉兆/凶兆？宗教仪式是如何利用它的？——这会催生出大量可落地的剧情事件（如：每十年一次的潮汐大典 = 宗门大比 + 资源拍卖会 + 敌对势力偷袭的绝佳时机）。

五、【主要势力总表（至少8个）】
至少列 8 个势力，按"宗门/王朝/帝国/家族/商会/教会/地下组织/异族联盟/学院派/军方"等不同性质分类，每一方写：
- 势力名称、性质、级别（王朝/宗门三级）、领袖、核心人物2-3人；
- 地盘（哪几个大区）、核心人口/兵员/弟子规模；
- 核心战力（宗主/国师/老祖什么境界、王牌军团/阵法/秘宝）；
...
- 中层：修士/军官/官员/富商/职业者 → 上升通道是什么？（科举？宗门考核？军功？捐官？商道？）
- 底层：凡人/矿奴/佃农/流民/佣兵/学徒 → 被盘剥的方式？翻身的概率？
- 禁忌群体：魔修/异族混血/偷渡者/贱籍/黑户 → 他们如何生存？主角是否曾属于这一类？
写出各阶层之间的流动是开是闭？有没有严格的种姓/血脉/出身限制？阶级矛盾是不是主线冲突的发动机？

七、【政治与律法（谁制定规则、谁执法、谁钻空子）】
- 国体：君主专制/贵族共和/宗门议会/联邦民主/神权/军政府？
- 法律的来源：神谕/皇帝敕令/宗门戒律/商会章程/旧例判例；
...
- 建筑风格：宗门洞府/皇宫/市井民居/店铺/书院/地下世界/异族营地——描写具体场景时要能对号入座。

十三、【军事与战力体系（组织/兵种/阵法/战争规模）】
如果有战争/国战/宗门战：
- 军事组织：禁军/边军/卫所/宗门护山队/骑士团/雇佣军/星舰舰队；
...
3) 归属势力：宗主国/统治宗门/占领军/割据家族/地下势力；
4) 核心建筑/地标：至少 3 处具体地标（城门/广场/大殿/学院/矿场/港口/传送阵/禁地）；
