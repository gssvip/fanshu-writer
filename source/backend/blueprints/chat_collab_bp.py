...
· 【铁律A·短段落≠碎句（禁令5配套·最高硬约束）】——短段落（1–3句成段）只为阅读节奏服务；**段落内的每一句都必须是完整、自然、可读的句子**（语义齐全连贯、停顿合理、主谓宾或逻辑链条齐全）。**绝不为求短而把一句话硬剁成几截读不通的残片**：反例1「他不说话。只是盯着赵屠子。」（虽各有句号，但语义是同一动作链的残切=碎句）→合并为「他不说话，只是盯着赵屠子。」；反例2「刀光一闪。血溅了半尺。」（同镜头连续动作的残切=碎句）→合并为「刀光一闪，血溅了半尺。」。宁可合并成2句完整的短句一段，也绝不写半句语义残缺的碎片段。
· 【禁令0·最高优先级·AI修正式否定句密集症】——**全章禁绝，出现=直接重写，无例外**。
  · 识别（任一条即命中）：①「不是X是Y/不是X。是Y/不是X准确说是Y/不是A也不是B——是C」否定-肯定交替句 ②嵌套≥2层修正句 ③排比三连/四连「更X，更Y，更Z」/看见…看见…/举起…落下…等动作排比 ④独立句号拆开的"不是X。是Y"句式 ⑤"但/至少不该/墨迹还没干透"类自我修正/抠字眼句。
...
· 【段落合并·硬执行（配禁令5段数硬上限+铁律A短段落≠碎句）】——**合并前置铁律**：合并或拆分时，不得为了凑"一句话一段"或"段数指标"强行把一句话中间切句号（主谓宾没说完、动作链没闭合就句号=残句/碎句）；任何句号切分后的句子本身必须语义完整，不得是同镜头连续动作的半段残片。相邻 3 句满足【同 POV + 同场景 + 同镜头】三同时，**必须至少合并 2 句成一段**；连续 ≥4 句一句话独立段（不管是不是对白）直接合并相邻 2-3 句：
...
· 【段落合并必过】相邻 3 句同 POV/同场景/同镜头有没有合并？连续 ≥4 句一句话独立段有没有合并相邻 2-3 句？段均长≥18字、极短句≤10字占比≤60%、2400字→段数 ≤106？有没有违反铁律A——为求短段把一句话硬剁成几截（如"他不说话。只是盯着赵屠子。""刀光一闪。血溅了半尺。"这种同镜头动作残切=碎句，必须合并成语义完整的单句/复句，不得留半句语义残缺的碎片段）？
...
def _action_chapter(book, session, instruction, gw, sse, target_chapter_num, prev_chapter_content, mode,
    ...
    if chapter_plot_ctx:
        chapter_plot_iron = (
            ...
            f'\n{chapter_plot_ctx}'
            '\n\n【本章字数铁律】纯正文（不含标题）2300-2500字，全角中文标点；不足扩场景细节/对话停顿/POV感官（温度/气味/触感/视线压力），超了删枝节。（⚠️【禁令0】不得用「不是X是Y/更X更Y更Z」修正句式/排比三连凑字）'
        )
...
禁令0·禁AI修正式否定句密集症（不是X是Y/排比三连/自我修正句）→【见行文规范禁令0细则，无例外】；禁令5·一句话一段话（漫画分镜病，段数硬上限）→【见行文规范段落合并细则】（以上两条已在下文写详细版，此处不复述）
...
禁令3·禁对白提示语句首≥50%+对白独立段：对白提示语（XX说/道/问…）放句首比例 ≤40%；**连续 3 句对白必须至少 1 句和动作/反应同段，严禁每句对白单独成段像剧本台词**。
