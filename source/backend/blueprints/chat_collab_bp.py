...
- 必删句式：不是……是……、不是……而是……、与其说……不如说……（⚠️ 例外：「不是 X，准确说是 Y」「不是痛→是凉→像冰碴」这类自我修正/感知修正句式保留——它是真人修正感写法，不是病；只砍"解释性排比的不是A是B三连"）
...
· ② 修正感句式："不是 X，准确说是 Y"=真人写法，鼓励；只砍解释性排比三连。
· ③ 感官细节三叠：温度/气味/触感/声音 选 2-3 叠（不用比喻词）。
· ④ 动作链=小目标链：每 200-300 字一个"小目标→决策→验证"闭环。
