...
5. 扫段落合并 → 【行文规范段落合并节】+【通用核心铁律·禁令5+铁律A】的全口径规则：三同时合并/连续4句独立段合并/段数硬上限/段均长/禁止残切碎句，细则不复述。
...
2. 段落句式两指标（段数硬上限/段均长）遵守【通用核心铁律·禁令5+铁律A段落≠碎句】，超标=按行文规范段落合并节主动合并，此处不再复述数值。
...
· 段落句式：段落（1–2句成段）可用于冲突/打架/对峙场景的爆发式密集节奏，但整章段数必须遵守「禁令5段数硬上限」（2400字→≤80段），段均长≥30字；超过=漫画分镜脚本化→必须合并相邻2-4句同场景/同镜头/同POV的叙述/动作/对白为一段。叙述主力段=40-90字（1-2个逗号长句）；叙述句主力形态=整句20-35字、逗号串1-2个动作单元收一个句号（占叙述句70-80%）；短句（≤15字）只做重拍、连续≤2句；叙述短段（＜40字）占比15-25%；对白段不限（一句一段是常态）。
...
· 【段落合并·硬执行（配禁令5段数硬上限+铁律A段落≠碎句）】——**合并前置铁律**：合并或拆分时，不得为了凑"一句话一段"或"段数指标"强行把一句话中间切句号（主谓宾没说完、动作链没闭合就句号=残句/碎句）；任何句号切分后的句子本身必须语义完整，不得是同镜头连续动作的半段残片。相邻 3 句满足【同 POV + 同场景 + 同镜头】三同时，**必须至少合并 2 句成一段**；连续 ≥4 句一句话独立段（不管是不是对白）直接合并相邻 2-3 句：
...
· 【段落合并必过】相邻 3 句同 POV/同场景/同镜头有没有合并？连续 ≥4 句一句话独立段有没有合并相邻 2-3 句？段均长≥30字、2400字→段数 ≤80？叙述句七成以上是不是逗号长句（20-35字、串1-2个动作）？连续 3 句 <12 字句号短句有没有合并？短句是不是只在转折/爆发/收尾做重拍（连续≤2句）？有没有违反铁律A——为求短段把一句话硬剁成几截（如"他不说话。只是盯着对面那人。""刀光一闪。血溅了半尺。"这种同镜头动作残切=碎句，必须合并成语义完整的单句/复句，不得留半句语义残缺的碎片段）？
