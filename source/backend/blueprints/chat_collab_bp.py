...
4. 禁止：不新增剧情/设定/观点/情绪/金句；不把所有句子都改成长句（冲突场景的爆发式密集短段=手机端阅读友好，允许保留；⚠️ 段数/段均长/极短句比例遵守【通用核心铁律·禁令5+铁律A短段落≠碎句】，漫画分镜脚本化=必须主动合并；不能堆砌无信息短句凑字数）；不堆砌"嗯啊吧嘛"假装口语化；不把解释腔改文艺腔/抒情腔；不输出修改说明、自检清单、协作口吻。
...
2. 对白AI味（第二大AI味）
   - 识别模式：角色像作者在解释剧情而非回应眼前人；自报动机/创伤/选择完整分析；对白硬塞世界观/规则/背景像说明书；冲突场景语气仍客气逻辑过顺；不同角色同句式同情绪；对白出现书面连接词"因此/然而/与此同时/换句话说"；**3句以上对白连续独立段不嵌动作/不夹反应（像剧本台词）**；**强压迫场景上下位者语气同质化=上位者客气+下位者完整对答**。
   - 处理：①保留必须剧情事实、关系变化 ②删角色不该明说的心理分析、主题总结、背景说明 ③调语气符合角色身份与当下情绪（不强行现代口语化）④制造缺口：用半句话/停顿/反问/打断替代完整解释 ⑤长对白拆短，中间用动作/停顿/对方插话承接 ⑥**强压迫场景/对白同段/3人声纹差规则 详见【正文写作阶段行文规范·对白节】，去AI阶段按同口径补合并动作/打断点 ⑦群像对白声纹差维度=用词/句长/语气词至少1维/2人**。
...
5. 扫段落合并 → 【行文规范段落合并节】+【通用核心铁律·禁令5+铁律A】的全口径规则：三同时合并/连续4句独立段合并/段数硬上限/段均长/极短句占比/禁止残切碎句，细则不复述。
6. 扫对白 → 【行文规范对白节】的全口径规则：替作者说解释删/强压迫场景语气差/连续3句至少1句动作同段/3人以上声纹差，细则不复述；发现命中直接补动作/打断点。
...
2. 段落句式三指标（段数硬上限/极短句占比/段均长）遵守【通用核心铁律·禁令5+铁律A短段落≠碎句】，超标=按行文规范段落合并节主动合并，此处不再复述数值。
...
· 【铁律A·短段落≠碎句（禁令5配套·最高硬约束）】——短段落（1–3句成段）只为阅读节奏服务；**段落内的每一句都必须是完整、自然、可读的句子**（语义齐全连贯、停顿合理、主谓宾或逻辑链条齐全）。**绝不为求短而把一句话硬剁成几截读不通的残片**：反例1「他不说话。只是盯着对面那人。」（虽各有句号，但语义是同一动作链的残切=碎句）→合并为「他不说话，只是盯着对面那人。」；反例2「刀光一闪。血溅了半尺。」（同镜头连续动作的残切=碎句）→合并为「刀光一闪，血溅了半尺。」。宁可合并成2句完整的短句一段，也绝不写半句语义残缺的碎片段。
...
· 【强压迫场景·压迫语气差·硬卡（矿场/监狱/宗门杂役殿/战场/衙门审问/奴隶市场/上级训话=必启用）】
  ...
  - 【上位者（监工/长老/军官/掌柜/审讯者）必写 3 选 1】
    a. 打断——下位者没说完就接话/喝止；
    b. 威胁式反问——不正面回应质疑，转成带具体后果的反问，把"是否执行"的压力甩回给对方；
    c. 用词粗硬——按题材粗细调整口癖度（严肃古题材用呵斥/命令词，现代市井题材可以用粗口短句，修仙宗门换成"聒噪""放肆""拖出去"等对应风格词）。
...
· 【段落合并·硬执行（配禁令5段数硬上限+铁律A短段落≠碎句）】——**合并前置铁律**：合并或拆分时，不得为了凑"一句话一段"或"段数指标"强行把一句话中间切句号（主谓宾没说完、动作链没闭合就句号=残句/碎句）；任何句号切分后的句子本身必须语义完整，不得是同镜头连续动作的半段残片。相邻 3 句满足【同 POV + 同场景 + 同镜头】三同时，**必须至少合并 2 句成一段**；连续 ≥4 句一句话独立段（不管是不是对白）直接合并相邻 2-3 句：
  - 叙述句接叙述句→同段（例："有人推了前面那人一把。他踩进泥水。"→合并成："有人推了前面那人一把，他踩进泥水，脚底滑过硬东西。"）。
...
· ④ 动作链=小目标链：每 200-300 字一个"小目标→决策→验证"闭环**（挖矿/跑步/赶路/抄写/练功/切菜/摆摊/跑图/任何重复性劳作/推进，全部纳入本条）**——**通用公式：小目标=当前阶段定量产出KPI → 决策=优化动作/路线/技巧以达成 → 验证=可量化的进度检查（做了多少/还差多少/工具或状态是否出问题）**；**不写"继续做/还在做/接着做"这种空转流水账，必须每 200-300 字给一个可验证的进度句**。
...
· 【段落合并必过】相邻 3 句同 POV/同场景/同镜头有没有合并？连续 ≥4 句一句话独立段有没有合并相邻 2-3 句？段均长≥18字、极短句≤10字占比≤60%、2400字→段数 ≤106？有没有违反铁律A——为求短段把一句话硬剁成几截（如"他不说话。只是盯着对面那人。""刀光一闪。血溅了半尺。"这种同镜头动作残切=碎句，必须合并成语义完整的单句/复句，不得留半句语义残缺的碎片段）？
...
禁令2·禁空转流水账重复动作：任何熬/抽/练/锻/淬/拼杀、挖矿/跑步/赶路/抄写/练功/切菜等任何重复动作 → 必须遵守【行文规范黄金4型·动作链】"小目标→决策→验证"每 200-300 字一个闭环，严禁只写"继续/还在/接着做"无验证句。
...
禁令6·禁强压迫场景对话语气同质化（矿场/监狱/宗门杂役殿/战场/衙门审问/奴隶市场=必启用）：上位者语气差+下位者语气差必须按规则写，上下位者客气礼貌对答=直接重写【详见行文规范对白节·强压迫场景语气差硬卡】。
禁令7·禁结尾真相直白点破：结尾为真相揭露时绝对禁止直接陈述句点破，必须改写成动态动作句收尾【详见行文规范结尾钩子·真相直白点破禁】。'''
