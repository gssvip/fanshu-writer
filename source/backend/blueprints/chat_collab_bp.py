...
- 必删排比：「更X，更Y，更Z」连续三连/四连排比（力度堆砌型，拆成自然单句推进）。
- 必删路标词：然而、同时、此外、更重要的是、总而言之、换句话说
- 必删协作口吻：下面我们、希望这能帮助、作为AI
- 结尾禁用：总结性、升华性、点题性语句（成长、命运、人性、选择、未来、从此、那一刻等拔高词）

【四、执行流程（写完/改完后逐条过一遍）】
1. 【第 1 条·先扫禁令0（最高优先级）】扫「不是X是Y/不是X。是Y/不是X准确说是Y/不是A也不是B——是C」全部形态 → 一律改写成直接陈述句。再扫「更X更Y更Z」排比三连 → 拆成自然单句。
...
2. 对白
· 禁：角色替作者讲设定/背景；自我剖析完整；冲突时仍客气过顺；书面连接词（因此/然而/与此同时）。
· 【对白 6 式·硬执行】
  ① 工整问答 → 答非所问/半句话/沉默/打断。
  ② 提示语全放句首 → 嵌句中或放句尾+小动作收（"XX说…"句首占比 ≤ 40%）。
...
· ④ 动作链=小目标链：每 200-300 字一个"小目标→决策→验证"闭环（用动作直接推进，不用"不是X是Y"句式抠字眼）。
...
· 【禁令0 必过】整章有没有任何形式的「不是X是Y/不是X。是Y/不是X准确说是Y/不是A也不是B——是C」？有没有排比三连「更X更Y更Z」？有没有"至少不该""但""墨迹还没干透"这种自我修正/抠字眼句式？→ **全部砍**，改成直接陈述句。
· 解释腔信号词有没有残留？
· 对白提示语句首占比 ≤40%？对白 6 式有没有至少用到 2-3 条？
· 多角色围观场景有没有写 ≥1 条递进比较链？
· 段落结构：主力段长 11-35 字、段均长 ≥18 字、极短句≤10 字占比 ≤60%、2400字→段数 ≤106？
...
def _action_chapter(book, session, instruction, gw, sse, target_chapter_num, prev_chapter_content, mode,
    ...
    # === 事前生成禁令·5条（反例矿道文暴露出的核心硬约束，第3高位，优先级最高档）===
    #   注入位置：core_iron → ONE主钩子 → 【事前禁令5条】 → style_pack → NARRATIVE_CRAFT_RULES
    PRE_GENERATE_BAN_RULES = '''【事前生成禁令·5条（比任何规则都高，违反直接扣分/重写）】
禁令1·禁背景板一次性名字段≥8段：群像围观（点卯台/集会/街道/战场围观群众）只允许1段集中描写（例："人群往后塌，麻衣沾泥的、缺半只耳朵的、抱着破布包的挤成一团"合并写）；严禁把 8 个以上路人/背景板角色各自拆成独立一段（王猛上前、老矿奴拉住、狗腿拔刀各一段这种→必须合成≤2段）。
禁令2·禁空转式"动作→突破"闭环：任何熬/抽/练/锻/淬/拼杀类场景，每 200-300 字必须有 1 个「小目标→决策→验证」小三段闭环（例：目标=吸完这鞭的煞劲→决策=咬着牙不哼一声顶过去→验证=大椎骨杂质少了三分，后颈出一层冷汗）；**没有闭环=不许写"大椎骨成/突破了/炼成了"**。
禁令3·禁对白提示语句首≥50%：全章所有对白提示语（XX说/道/问/喝/冷道/笑道…）里，放在对白句子**前面**（例："黑狗说：抽死你！"）的比例必须 ≤40%；≥60% 要嵌在对白中或对白后（例：""抽死你！"黑狗牙咬得咯吱响，鞭子抡到最高。"）。
禁令4·禁群像平级并列段：点卯台/围观/集会/站队类多角色场景，必须有≥1条「递进比较链」（A→B→C→D分层：例"矿奴退得比王猛快→王猛比黑狗前的狗腿还横→狗腿在黑狗面前像根木头→黑狗见了赵屠子会直接跪→1字短句收尾：狗腿的刀哐当掉地上。"）；**不许 3 段以上"A做了→B做了→C做了"平级并列堆段**。
禁令5·段数硬上限卡：按目标字数÷25字/段=段落数上限（例：2400字目标=≤96段）；真人写作为40-80段/章；超过96段=漫画分镜脚本化，必须把相邻2-4句同场景、同POV、同镜头的叙述/动作/对白合并成一段。'''
    # 算一下目标段落数（用于禁令5）：按目标字数 / 25（真人平均段长略高，取25更宽容），取整+10的浮动空间
    TARGET_WORDS = 2400
    para_count_cap = int(TARGET_WORDS / 25) + 10  # 2400/25+10 = 106
    PRE_GENERATE_BAN_RULES += f'\n（本章目标字数 {TARGET_WORDS}→段落数硬上限 ≤{para_count_cap} 段）'
