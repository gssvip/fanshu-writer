...
def _action_chapter(book, session, instruction, gw, sse, target_chapter_num, prev_chapter_content, mode,
    ...
    # === 事前生成禁令·4条（反例矿道文暴露出的核心硬约束，第3高位，优先级最高档）===
    #   注入位置：core_iron → ONE主钩子 → 【事前禁令4条】 → style_pack → writing_rules
    #   ⚠️ 禁令5（段数硬上限）已移到 GENERAL_CORE_RULES 总则里全阶段通用，这里不再重复写 2 遍。
    PRE_GENERATE_BAN_RULES = '''【事前生成禁令·4条（比任何规则都高，违反直接扣分/重写）】
禁令1·禁背景板一次性名字段≥8段：群像围观（点卯台/集会/街道/战场围观群众）只允许1段集中描写；严禁把 8 个以上路人/背景板角色各自拆成独立一段（王猛上前、老矿奴拉住、狗腿拔刀各一段这种→必须合成≤2段）。
禁令2·禁空转式"动作→突破"闭环：任何熬/抽/练/锻/淬/拼杀类场景，每 200-300 字必须有 1 个「小目标→决策→验证」小三段闭环；**没有闭环=不许写"大椎骨成/突破了/炼成了"**。
禁令3·禁对白提示语句首≥50%：全章所有对白提示语（XX说/道/问/喝/冷道/笑道…）里，放在对白句子**前面**的比例必须 ≤40%；≥60% 要嵌在对白中或对白后。
