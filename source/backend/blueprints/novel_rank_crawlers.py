"""榜单风向 · 抓取适配器与种子数据（从 novel_rank_bp.py 拆出，架构门禁：单文件 ≤2000 行）。

包含：
1. 榜单种子数据：2 站点 × 232 分类 × 83 榜单源（番茄 / 起点；七猫已按产品要求下线）
2. HTTP 会话工具：TLS 兜底 / 重试 / 共享会话 / 番茄私用区字体解码
3. 平台抓取适配器：番茄 HTML + API / 七猫 JSON / 起点移动端 JSON 接口
4. 1h 内存缓存 + 熔断聚合：crawl_rank_source
5. 榜单源解析：resolve_sources

本模块不依赖 Flask，可独立导入单测；路由层见 blueprints/novel_rank_bp.py。
"""
