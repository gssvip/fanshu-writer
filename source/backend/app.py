...
SEED_SKILL_PACKS = [
    ...
    # ==== 墨魂清音：源自 blader/humanizer (30.9k stars)，专业去AI味 ====
    {'name': '墨魂清音', 'description': '专业去AI味技能包，源自 GitHub 30.9k stars 的 humanizer 项目。识别并清除33种AI写作痕迹，保留信息而非保留形状，让文字回归自然有魂。',
    ...
    # ==== 真相之书：源自 Narcooo/inkos (8.3k stars)，多Agent流水线 ====
    {'name': '真相之书', 'description': '多Agent协作写作系统，源自 GitHub 8.3k stars 的 InkOS 项目。五类Agent分工（建筑师/写手/审计员/修订员/文风工程师），7个真相文件防幻觉，33维度审计。',
     'genre': 'other', 'book_type': 'novel', 'icon': '📜',
    ...
    {'name': '长篇铁律', 'description': '长篇小说四阶段工作流，源自 yingzhu77/my-skills 基于180章114万字符实战经验。防止结尾模板化/身体反应固化/食物描写机械化/状态表膨胀等系统性问题。',
     ...
     'prompts': json.dumps({
         ...
         'consistency_check': '你是长篇一致性检查器。每5章运行一次，6个维度：\n【六维检查】1)资源跳变：食物/物资/药品不能无故减少增加 2)伏笔重复：同一伏笔ID不能被使用两次 3)时间线倒退：天数只能向前推进 4)角色位置：角色不能同时出现在两个地方 5)字数：章节不能低于6500字符上限 6)缺失食物来源：每章必须提及食物来源。\n【输出格式】PASS: ch001-ch005 / FAIL: ch006 - 食物从30跳到50但无交易场景 / PASS: ch007-ch010。\n【集成方式】每5章运行，所有检查必须PASS才能继续写下一章。\n【8种反模式扫描】1)结尾模板(每章日志+食物+总结) 2)环境重复(每章同一扇窗) 3)比喻复用(不同角色相同比喻) 4)反应固化(总是呼吸一滞) 5)食物库存化(配给变会计) 6)三重重复(对话+心理+日志) 7)日志当安全床(默认结尾) 8)中段公式(危机→谈判→临时修复→新威胁循环)。',
         'volume_revision': '你是卷末修订员。每卷(30章)结束后运行风格修订：\n【修订四步】1)扫描重复结尾/句式/比喻 2)修复模板化模式(连续3+章相同结尾结构) 3)多样化角色身体反应(10章内同种反应≤2次) 4)移除章节末尾信息冗余。\n【每10章风格检查清单】字数与场景深度(每章≥4000字)/结尾检查(与最近2章不同)/重复句式(10章内3+次替换)/身体反应频率(呼吸一滞10章≤2次)/比喻追踪(每个比喻只用一次)/角色出场(主角10/10核心7-10配角3-5)。\n【状态管理】状态表只保留最近3章详细信息，更早压缩为单行摘要。伏笔表分Active(下10章关注)/Archived(已回收或搁置)，写作agent只读active。\n【确认门】主要角色死亡/背叛/关系变化、世界规则改变、卷与卷过渡、每10章回顾——需停下询问。\n【交付前清单】推进剧情/人物/世界至少一项；有明确冲突和信息增量；字数4000-6500；2+完整场景；对话有节拍无5+连续问答；2+内心独白；每场景2+感官；1+主题化环境；结尾与最近2章不同；无最近5章重复身体反应；食物场景化非库存化；同一事实不在对话/心理/日志三者全有；写作日志含创作决策。',
     }, ensure_ascii=False)},

    # ==== 史诗铸魂：源自 gabremoku/fantasy-fiction-writer，奇幻史诗写作 ====
    {'name': '史诗铸魂', 'description': '史诗奇幻写作技能包，源自 gabremoku/fantasy-fiction-writer。融合托尔金神话深度+Troisi情感直接性+Martin结构纪律，内置9种反AI散文审计层。',
