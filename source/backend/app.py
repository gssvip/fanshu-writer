...
SEED_SKILL_PACKS = [
    ...
    {'name': '玄幻小说文风', 'description': '【文风专用】冷静克制硬朗文风：短句密集+对话驱动+禁词管控+长短句比例控制，玄幻正文专用文风锚定', 'genre': 'fantasy', 'book_type': 'novel', 'icon':'⚔️',
     'stage_keys': json.dumps(['draft'], ensure_ascii=False),
     'workflow': json.dumps([
     ...
     'prompts': json.dumps({
         'style_anchor': '【文风阶段·锚定】你是玄幻文风锚定师。根据参考文本或偏好，生成7维文风速查表：1)整体风格(冷静/克制/硬朗) 2)长短句比例(短句40%-60%每句1-20字，中句20%-30%每句20-35字，长句10%-20%每句35-70字) 3)对话占比(单章25%-60%) 4)叙述规则(描写服务情报和动作，不堆氛围) 5)信息推进方式(对话式推理优先) 6)章法要求(开头进场景，结尾留钩子) 7)禁用倾向(大段心理独白、形容词堆砌、总结性句子)。每维一行，创作时照着写。',
         'fantasy_draft': '【文风阶段·写正文】你是玄幻小说写手。严格遵循以下文风铁律写作：\n【核心风格】冷静、克制、硬朗。短句密集，信息推进快。对白推动剧情，叙述只做必要交代。少抒情，少形容词堆砌，少大段心理描写。用动作、物象和对白表达情绪。悬念靠信息缺口、人物反应和新线索推进。\n【长短句比例】短句40%-60%(1-20字)用于回答/反问/动作/判断/悬念落点；中句20%-30%(20-35字)用于解释规则/交代背景/补充推理；长句10%-20%(35-70字)只用于复杂设定说明或必要场面描述。超过70字的句子必须拆开。\n【对话占比】单章对白段落占比25%-60%。推理章可提高到60%。每轮对白只推进一个信息点。避免长篇独白。\n【叙述规则】描写要服务情报、动作和悬念。不堆氛围，不写泛泛的宏大抒情。人物情绪优先用动作体现。心理活动保持短促，避免反复解释。场景描写先给位置，再给关键物，再进入行动。\n【章法要求】每章开头尽快进入场景或事件。每章中段用对白和行动推进信息。每章结尾落在新风险、新线索或新决策上。不用空泛总结收尾。不用连续大段设定说明。\n【推荐结构】动作或场景一句→对白一句→短反应一句→继续对白→信息揭示一句。\n【禁用倾向】避免大段心理独白；避免连续堆形容词；避免用"震惊、复杂、激动"直接总结情绪；避免一段里同时写多个动作、解释和心理；避免超过70字的长句；避免使用总结性、评价性、升华的句子。\n输出2400字±100正文。',
         'forbidden_words': '【文风阶段·禁词】你是禁词扫描员。逐句检查文本中是否出现以下禁词，发现即替换：\n【禁词清单】一股、一抹、不由得、不禁、随即、旋即、与此同时、颇为、甚为、极为、缓缓、淡淡、轻轻、微微、毫无疑问、毋庸置疑、不言而喻、深吸一口气、眼中闪过一丝、心中暗想、心念电转、若有所思、不知不觉间、转眼间、恍然大悟、面无表情、淡漠、漠然、眸子、嘴角微微上扬、如同、宛如、犹如、周身、周遭、气息、威压、那道身影、说话间、话音未落、当即、顿时、瞬时、因此、然而、显而易见、由此可见、总而言之、综上所述、震惊、复杂、激动。\n【替换表】一股暖流涌上心头→胸口一热/鼻子一酸；嘴角微微上扬→嘴角一翘/咧嘴；恍然大悟→哦了一声/拍了下大腿；深吸一口气→吐了口气/咬了咬牙；淡淡地说道→说/应了一声；不由得倒吸一口凉气→嘶了一声/愣住了；眼中闪过一丝寒芒→眼神一冷/目光像刀子；若有所思地点点头→点点头没说话；心中暗想→直接写内心OS；周身散发着强大气息→走过来的时候空气都变沉了。\n只输出替换后的正文，不解释改了什么。',
         'rhythm_check': '【文风阶段·节奏】你是句式节奏检查员。逐段检查以下维度：1)短句是否占40%-60%(1-20字)？2)中句是否占20%-30%(20-35字)？3)长句是否控制在10%-20%(35-70字)？4)是否存在超过70字的句子？5)对白段落是否约占25%-60%？6)每轮对白是否只推进一个信息点？7)章尾是否留下明确钩子？8)是否避免了总结性、评价性、升华的句子？9)是否避免了大段心理独白和形容词堆砌？10)描写是否服务情报、动作和悬念？11)有没有可以用动作代替的心理描写？12)内心独白是否控制在5%以内？输出每项的通过/未通过状态和具体位置。',
     }, ensure_ascii=False)},
    ...
    {'name': '都市异能文风', 'description': '【文风专用】都市异能正文文风：现代口语+爽快节奏+异能展示与日常切换，番茄都市异能专用文风锚定',
     ...
     'prompts': json.dumps({
         'style_anchor': '【文风阶段·锚定】你是都市异能文风锚定师。生成7维文风速查表：1)整体风格(现代/口语化/爽快) 2)长短句比例(短句45%-60%每句1-20字，中句20%-30%每句20-35字，长句10%-15%每句35-70字) 3)对话占比(单章25%-60%，现代口语+网络用语适度) 4)叙述规则(异能展示服务剧情，不堆设定) 5)信息推进(对话+动作双驱动，异能效果靠画面感) 6)章法(开头进事件，结尾留悬念或打脸) 7)禁用(古风词汇/文绉绉表达/大段心理独白)。',
         'genre_style': '【文风阶段·写正文】你是都市异能小说写手。严格遵循以下文风铁律：\n【核心风格】现代、口语、爽快。短句密集，节奏明快。对白推动剧情，异能展示靠画面。少抒情，少堆形容词，少大段心理。用动作、对话和异能效果表达情绪。\n【长短句比例】短句45%-60%(1-20字)用于对话/动作/反应；中句20%-30%(20-35字)用于交代背景/异能规则；长句10%-15%(35-70字)只用于复杂异能场面。超70字必须拆开。\n【对话占比】单章25%-60%。现代口语，适度网络用语("卧槽/牛逼/拉倒/得了吧")，但不过度。每轮对白推进一个信息点。\n【叙述规则】异能展示服务剧情不堆设定。日常场景与异能场景切换自然，靠"事件触发"过渡而非旁白解释。人物情绪用动作体现(攥拳/咬牙/后退一步)。\n【章法】开头尽快进入事件(冲突/异能触发/对话)。中段用对白和动作推进。结尾落在新危机/新异能觉醒/新反派登场。不用空泛总结收尾。\n【禁用倾向】古风词汇(汝/尔/且慢/此乃)；文绉绉表达(不禁/缓缓/淡淡)；大段心理独白；连续堆形容词；总结性升华句子。\n【都市感】场景要有现代感(地铁/写字楼/便利店/夜市)，异能要融入日常(隐于市/打工异能者/异能事务所)。\n输出2400字±100正文。',
     }, ensure_ascii=False)},
