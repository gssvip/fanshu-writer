...
# P0-3：静默降级治理 —— 用 safe_import 统一封装可选组件导入，降级可观测（FANSHU_STRICT=1 时抛异常）
from safe_degrade import safe_import

# P0-1：确定性后写校验器（零 LLM 成本，章节生成后检测 AI 痕迹）
_pwv = safe_import('post_write_validator',
                   ['validate_chapter', 'validate_chapter_with_bible', 'get_repair_hints',
                    'validate_chapter_with_drift', 'compute_style_fingerprint'],
                   label='P0-1 后写校验器')
validate_chapter = _pwv['validate_chapter']
validate_chapter_with_bible = _pwv['validate_chapter_with_bible']
get_repair_hints = _pwv['get_repair_hints']
validate_chapter_with_drift = _pwv['validate_chapter_with_drift']
compute_style_fingerprint = _pwv['compute_style_fingerprint']

# P0-2：伏笔 DAG 管理器
_fdm = safe_import('foreshadowing_manager',
                   ['parse_text_to_dag', 'ForeshadowingGraph', 'get_hooks_for_chapter'],
                   label='P0-2 伏笔 DAG')
parse_text_to_dag = _fdm['parse_text_to_dag']
ForeshadowingGraph = _fdm['ForeshadowingGraph']
get_hooks_for_chapter = _fdm['get_hooks_for_chapter']

# P1-4：四级大纲层级构建器
_ohb = safe_import('outline_hierarchy_builder',
                   ['build_outline_hierarchy', 'build_dramatic_position_prompt', 'get_dramatic_context'],
                   label='P1-4 四级大纲构建器')
build_outline_hierarchy = _ohb['build_outline_hierarchy']
build_dramatic_position_prompt = _ohb['build_dramatic_position_prompt']
get_dramatic_context = _ohb['get_dramatic_context']

# P1-5：节拍模板加载器
_btl = safe_import('beat_template_loader', ['build_beat_prompt'], label='P1-5 节拍模板')
build_beat_prompt = _btl['build_beat_prompt']

# P1-6 + P1-7：章级变更处理器（12类CHANGES解析 + delta回写）
_ccp = safe_import('chapter_changes_processor',
                   ['extract_changes', 'apply_chapter_changes', 'remove_chapter_changes',
                    'build_changes_prompt_template'],
                   label='P1-6/7 章级变更处理器')
extract_changes = _ccp['extract_changes']
apply_chapter_changes = _ccp['apply_chapter_changes']
remove_chapter_changes = _ccp['remove_chapter_changes']
build_changes_prompt_template = _ccp['build_changes_prompt_template']

# P2-9：Spot-Fix 修订路由器
_rev = safe_import('revise',
                   ['route_revision', 'build_spot_fix_prompt', 'apply_spot_fix_patches', 'estimate_token_saving'],
                   label='P2-9 Spot-Fix 修订路由器')
route_revision = _rev['route_revision']
build_spot_fix_prompt = _rev['build_spot_fix_prompt']
apply_spot_fix_patches = _rev['apply_spot_fix_patches']
estimate_token_saving = _rev['estimate_token_saving']

# P2-8：审计-修订闭环
_rc = safe_import('chapter_review_cycle',
                  ['run_review_cycle', 'run_review_cycle_with_bible', 'PASS_SCORE'],
                  label='P2-8 审计-修订闭环', fallbacks={'PASS_SCORE': 85})
run_review_cycle = _rc['run_review_cycle']
run_review_cycle_with_bible = _rc['run_review_cycle_with_bible']
PASS_SCORE = _rc['PASS_SCORE']

# P2-10 + P2-11：落地门禁 + PRE_WRITE_CHECK
_gg = safe_import('generation_gate',
                  ['run_all_gates', 'build_pre_write_check_prompt'],
                  label='P2-10/11 落地门禁')
run_all_gates = _gg['run_all_gates']
build_pre_write_check_prompt = _gg['build_pre_write_check_prompt']

# P3：LLM Gateway 统一入口（错误分类 + 智能重试 + 空内容检测）
def _fallback_get_output_limit(base_url: str, model: str) -> int:
    """导入失败时的回退实现：未知上限，不钳制。"""
    return 0

def _fallback_learn_output_limit(base_url: str, model: str, error_text: str, requested: int) -> int:
    """导入失败时的回退实现：不解析。"""
    return 0

def _fallback_build_auth_headers(api_key: str, content_type: bool = True) -> dict:
...
_lg = safe_import('llm_gateway',
                  ['LLMGateway', 'ModelResult', 'FailureClass', 'LLMError', 'get_llm_config',
                   'create_gateway', 'build_auth_headers', 'get_output_limit', '_learn_output_limit'],
                  label='P3 LLM Gateway',
                  fallbacks={
                      'get_output_limit': _fallback_get_output_limit,
                      '_learn_output_limit': _fallback_learn_output_limit,
                      'build_auth_headers': _fallback_build_auth_headers,
                  })
LLMGateway = _lg['LLMGateway']
ModelResult = _lg['ModelResult']
FailureClass = _lg['FailureClass']
LLMError = _lg['LLMError']
get_llm_config = _lg['get_llm_config']
create_gateway = _lg['create_gateway']
build_auth_headers = _lg['build_auth_headers']
get_output_limit = _lg['get_output_limit']
_learn_output_limit = _lg['_learn_output_limit']

# P3：Context Manifest（章节生成前记录上下文来源 + hash + token 预算）
_cm = safe_import('context_manifest', ['ContextManifest', 'ContextOrchestrator'], label='P3 Context Manifest')
ContextManifest = _cm['ContextManifest']
ContextOrchestrator = _cm['ContextOrchestrator']

# P3：PromptSpec 编译器（prompt 可测试 + golden 断言）
_ps = safe_import('prompt_spec', ['PromptSpec', 'PromptCompiler', 'load_prompt_spec'], label='P3 PromptSpec')
PromptSpec = _ps['PromptSpec']
PromptCompiler = _ps['PromptCompiler']
load_prompt_spec = _ps['load_prompt_spec']

# P4：运行恢复（幂等键 + 单步 retry/resume）
_rr = safe_import('run_recovery', ['IdempotencyKey', 'RunRecoveryService'], label='P4 运行恢复')
IdempotencyKey = _rr['IdempotencyKey']
RunRecoveryService = _rr['RunRecoveryService']
...
try:
    ...
    from blueprints.auth_bp import auth_bp; app.register_blueprint(auth_bp)  # 认证域（register/login/密码重置/VIP，拆自 app.py）
    from blueprints.export_bp import export_bp; app.register_blueprint(export_bp)  # 导入导出域（txt/docx/epub/zip/封面/文件导入，拆自 app.py）
    from blueprints.books_bp import books_bp; app.register_blueprint(books_bp)  # 作品/章节/人物/大纲 CRUD（拆自 app.py）
    from blueprints.dynamic_reports_bp import dynamic_reports_bp; app.register_blueprint(dynamic_reports_bp)  # 动态报告域（含辅助函数+11路由，拆自 app.py）
    from blueprints.ai_continue_bp import ai_continue_bp; app.register_blueprint(ai_continue_bp)  # 正文滚动创作域（续写/定点修订/批写/SSE，拆自 app.py）
    from blueprints.ai_analyze_bp import ai_analyze_bp; app.register_blueprint(ai_analyze_bp)  # AI 分析域（9+2 个 ai-analyze 路由，拆自 app.py）
...
# 动态报告服务函数已拆到 blueprints/dynamic_reports_bp.py；此处保留 app 命名空间向后兼容，
# 供 books_bp.py / export_bp.py 及 app.py 内章节保存/ai_continue 等请求期 `from app import ...` 延迟导入。
from blueprints.dynamic_reports_bp import (
    _check_and_auto_generate_report,
    _auto_backfill_dynamic_reports_async,
)

# 正文滚动创作域已拆到 blueprints/ai_continue_bp.py；此处保留 app 命名空间向后兼容，
# 供 chat_smart_edit_bp.py（`from app import _consistency_check`）及测试
# （test_deai_injection_runtime / test_ai_continue_fingerprint 经 `app.X` / `from app import X`）引用。
from blueprints.ai_continue_bp import (
    _consistency_check,
...
# ==== 数据模型（外迁至 models.py：db 实例在 extensions.py 统一 init_app，模型类向上兼容导出）====
from models import (
    User, AuthToken, PasswordResetToken,
    Book, Chapter, ChapterVersion, Character, Outline, DailyStats, Template,
    AISession, AIConfig, AIUsageLog, AppPreference, StageContent, BookBible,
    SkillPack, DynamicMemory, DynamicReport, PromptTemplate, AppMeta,
)

# ==== 鉴权工具（拆至 auth_utils.py：app.py 58 处路由 + 各蓝图共用，单向导入无循环）====
...
【节点 7 要素铁律（每个节点必须齐全，只写这七项，不做任何文字润色）】
节点是"剧情调度卡"，不是小说正文、也不是大纲抒情段。每个节点只许包含以下七项结构要素，一律用最干的分句写，禁止堆砌：
  · time：本节点时间锚（相对卷级 + 绝对章号，如"卷首第3周·第4章当晚三更"）
  · location：本节点发生地点（精确到场景，如"黑骨矿场·丙字采区·第7巷道口"）
  · events：本节点事件（剧情句：谁在什么场景做了什么→关键后果，20-40字）
  · conflict：本节点冲突（本节点内的对立/矛盾：主角 vs 谁、争什么、卡点是什么，20字内；没有独立冲突则写它承接的上层核心冲突，绝不留空）
  · characters：本节点核心出场人物（按权重排序，含小角色）
  · hook：本节点章尾钩子（七种钩子之一，见【钩子系统】）
  · 伏笔：本节点埋/收的伏笔（即 bury / payoff 字段）
【节点负清单（七项之外的文字一律不写）】
1. 禁止环境/物象描写：不写天气、街景、陈设、光线、气味、物件清单等营造氛围的句子。
2. 禁止比喻/拟人/排比/工整对仗等修辞，禁止"如同/宛如/仿佛/整一千钧之势"类修饰。
3. 禁止动作细节链与形容词堆砌：不写"后退半步、猛然抬臂、青色筋络暴凸"这类碎动作；形容词（苦笑、阴沉、暴戾、废旧、惨烈等）全部清空。
4. 禁止心理/情绪铺陈：不写"暗暗发狠/心头一凛/怒不可遏"等内耗描写。
5. summary 只做事件推进梗概（起因→关键动作→直接后果→收尾→钩子各一句，动词+名词为主），不得出现上述任何修饰性文字。

【章粒度铁律·chapter_beats（本节点跨 N 章的剧情切片，A方案核心）】
· 节点 chapters 是多章区间（如 6-10 共5章）时，必须按每章一条拆出 chapter_beats 数组，数组长度 = 区间章数，第 i 条对应该区间内的第 i 章。
· 格式 = {{"chapter": 章号, "beat": "本章只推进的这一段剧情（20-35字：谁做了什么→到达什么状态），必须在该章内完成且不触碰下一章的剧情"}}。
· 分章原则：把节点整段剧情按"单章能写完、且不透支下一章"的标准，顺序切成若干段。每章的 beat 只能推进本章内容，把高潮/反转/收尾/钩子按章位顺延分配，**严禁把整段剧情塞进某一条 beat**。
· 若该区间内某章为纯过渡/铺垫章，beat 写推进动作与递进状态即可，不必每章都有爆点。
· summary 仍是整段推进梗概，chapter_beats 是它的按章细分；两者必须一致，chapter_beats 的总和恰好覆盖 summary。
...
{{
  ...
  "nodes": [
    {{
      ...
      "events": "本节点事件：谁在什么场景做了什么→关键后果（20-40字，动词+名词，禁修饰）",
      "conflict": "本节点冲突：主角vs谁/争什么/卡点（20字内；继承上层核心冲突则写明承接源）",
      "time": "本节点时间锚（相对卷级+章号）",
      "location": "本节点精确场景地点",
      "realm_change": "本节点结束时境界/根基/金手指变化",
      "age_change": "本节点结束时主角年龄/时程变化",
      "summary": "本节点事件推进梗概（起因→关键动作→直接后果→收尾→钩子各一句，纯动词+名词，须支撑本节点 chapters 区间按每章2400字估算的容量；禁止环境/比喻/形容词/心理铺陈）",
      "chapter_beats": [{{"chapter": <区间内第1章章号>, "beat": "本章只推进的这一段剧情（20-35字）"}}, {{"chapter": <区间内第2章章号>, "beat": "..."}}],
      "cool_type": "爽点类型（八选一）",
...
# 正文滚动创作域（blueprints/ai_continue_bp.py）的函数体依赖本文件大量全局名
