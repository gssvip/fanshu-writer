...
SEED_SKILL_PACKS = [
    ...
    {'name': '去AI味儿改稿心法', 'description': '【审查专用】四步去AI味：查痕迹→最小改写→补人味→终检，保留作者风格不被AI抹平', 'genre': 'other', 'book_type': 'short_story', 'icon':'🎭',
     ...
     'prompts': json.dumps({
         'detect_ai': '【审查阶段·检测】你是AI写作痕迹检测专家。逐段扫描文本，定位以下10种AI特征并输出具体行号+问题描述：①连续排比句≥3句 ②模板过渡词（然而/不过/不禁/不由得/随即/与此同时等）密集 ③段落长度过于均匀（长短差<30%） ④情感直述而非展示（"他很悲伤" vs 动作细节） ⑤段尾总结本段内容 ⑥信息倾泻大段设定解说 ⑦对话过于书面化无个性 ⑧缺少口语语气词和断句 ⑨转场过于工整模板化 ⑩形容词堆叠。',
         'minimal_rewrite': '【审查阶段·改写】原则：编辑文字，不抹去文字背后的人。只修改被检测出AI痕迹的位置，保持作者原意和文风不变：①把模板句式改成自然口语表达 ②打散过于均匀的段落节奏（故意制造长短差） ③把情感直述句改为行为/动作/细节展示 ④删除所有段尾总结句 ⑤对话加语气词、打断、结巴、重复。其余内容一个字都别动。',
...
## 句子结构（爆款校准：叙述句七成以上是逗号长句，短句只是偶尔的重拍）
- 叙述句主力形态：整句 20-35 字（含标点），逗号串联 1-2 个动作单元收在一个句号里，占叙述句 70%-80%。例："他踩进泥水，脚底滑过硬东西。""她把合同折成四折塞进包里，指节压得很白。"——一个句号交代完一个完整动作画面，不拆成三个短句。
- 短句（≤15字）：占叙述句 20%-30%，只做重拍——用于关键转折、情绪爆发点、段落收尾落点，不连续使用超过 2 句。
- 对白句不受句长限制（口语天然短）。
- 超过 55 字的叙述句必须拆开。
- 碎句判定：连续 3 句以上句号收尾的 <12 字短句 = 漫画分镜式碎句，必须合并成逗号长句。

## 对话占比
- 单章对白段落占比保持25%-60%。推理章/情报章可提高到60%，场面章可降到25%。
- 避免长篇独白。每轮对白只推进一个信息点。
...
- 感官配额：每千字至少 2 处非视觉感官（触觉/听觉/嗅觉/温度/疼痛/气味），爽点与冲突段优先落感官；通篇只有"看见"=干燥播报。
...
## 节奏呼吸
- 每 2 个冲突波之间留半段闲笔（环境一笔/小动作/一句废话），全程紧绷无喘息 = 节奏灾难。
- 喘息段同时承担信息增量或伏笔，不是纯废笔。
...
## 禁词清单（严禁使用，命中即改，无例外）
【AI 必删词 30+（突然/忽然/猛地类 + 仿佛类 + 内心类 + 叙述连接类）】
突然、忽然、猛地、骤然、蓦然、蓦地、陡然、仿佛、好像、似乎、宛若、犹如、
恍若、宛如、好似、类似、不禁、不由得、忍不住、下意识、竟然、居然、
简直、不禁、竟自、暗自、心中一动、心里咯噔、心里一沉、心中暗道、
心里盘算、心头一震、心中一凛、不由得一怔、
【AI 叙述连接词】
一股、一抹、随即、旋即、当即、顿时、瞬时、与此同时、颇为、甚为、极为、
毫无疑问、毋庸置疑、不言而喻、深吸一口气、眼中闪过一丝、心中暗想、
心念电转、若有所思、不知不觉间、转眼间、恍然大悟、面无表情、
淡漠、漠然、眸子、嘴角微微上扬、如同、周身、周遭、
气息、威压、那道身影、说话间、话音未落、
因此、然而、显而易见、由此可见、总而言之、综上所述、
【Humanizer·删废话黑名单 16 词·命中即改，无例外】
值得注意的是、总的来说、与此同时、不可否认、毫无疑问、显而易见、
众所周知、由此可见、值得一提的是、综上所述、换言之、从某种意义上说、
需要指出的是、不言而喻、毋庸置疑、也就是说、换句话说、不难看出、
可以说、可以这么说、需要说明的是
【8 大 AI 比喻禁词·大海/巨龙/深渊/星河搭配版（命中即改原创比喻）】
宛如/犹如/恍若/宛若/如同 + 大海/汪洋/巨龙/长龙/深渊/星河/星河般/洪流/潮水
【禁令0·修正式否定句·5类识别口径（全章禁绝，命中一律改直接陈述句）】
1. 对立式否定（"不是A而是B""不是X，是Y""不是A，却B"）
2. 排比三连修正式（"不是…不是…而是…""不是A不是B不是C，是D"）
3. 自我修正式（"或者说…""甚至可以说…""准确地讲…""更准确地说…""与其说…不如说…"）
4. 否定式修辞（"不是比喻""不是夸张""不是地震那种，是…"——先否定再肯定）
5. 否定词前置的修饰腔（"不夸张地说…""不客气地讲…""不夸张地形容…"）
【禁令0·执行铁律（输出前必须逐条自查；命中 1 处整章不合格，必须在输出前自行替换为下面的改写，不许保留「不是/是」「与其说/不如说」「更准确地说」原句连接词）】
5 种改写公式（按优先级选，从上到下能套就套）：
① 动作实证法：A 是抽象判断（这不是梦 / 不是错觉 / 不是幻觉）→ 删掉全句，只写 1 个能证明 Y 的具体动作（掐手背十七次 / 指甲盖里留血痂 / 鞋底磨出白印 等）
② 物象替代法：A 是读者先脑补的普通物（不是震动 / 不是普通的疼 / 不是普通的光）→ 删掉否定句，直接摆"和脑补物完全不一样的物的细节"（地面没抖 / 指甲刻能出印 / 光不晃眼却晒得后颈脱皮）
③ 转对白碎嘴法：A 是作者想让读者搞清的关系（不是他数的 / 不是他发现的）→ 改成旁边旁观者碎嘴的对白或主角自言自语一句，让人物自己开口讲，不用作者站出来做纠正解说
④ 果先因后法：A 是原因解释（不是因为他发现了什么 / 不是因为他怕）→ 先写"已经发生的结果（鞭子停了 / 他钉住没动）"，紧跟着写 1 个具体细节当 C，细节一写读者自然就知道原因是 C，不用对比否定
⑤ 独立重拍段法：A 是"读者会误解"（不是被压制 / 不是因为他怕）→ 拆成 2 个独立小短段，第一段只放"看似 A 的表象"（他想挪脚 / 他攥紧了拳头），第二段放"B 的实锤"（腿不听使唤 / 指甲掐进掌心出白印），两段之间留白，绝对不许写「不是/是」「与其说/不如说」连接词
4 套抄作业对译表（如果写出跟左边一模一样的句式，输出前直接抄右侧对应版本，不用再想）：
T1「不是他数的，是旁边那个老六的役卒数的。老六每搬一块就在腰带上划一道。」→ 直接改写：「旁边的老六在腰带上面划一刀，嘴里嘟囔："十七，还剩八百四十三刀。"赵政没问，鞭子已经落了下来。」（公式③ 转对白碎嘴）
T2「前两天他一直在确认一件事：这不是梦；第三天他确认了另一件事：他有一个系统。」→ 直接改写：「头两天他把手背掐了十七次，指甲盖里留了血痂。第三天摸到手里磨毛的石头——右上角亮了行字。」（公式① 动作实证）
T3「不是震动，是一种更深的感觉，像是某根沉睡了很久的筋骨突然绷紧。」→ 直接改写：「地面没抖。地骨有一根筋抽了一下。从脚心沿胫骨往上爬，到腰，散。」（公式② + ⑤ 物象替代+独立重拍段）
T4「不是因为他发现了什么，是因为鞭子突然变轻了。他甩了两下，皮鞭软塌塌地落在地上，没有任何术法的重量。」→ 直接改写：「陈福的鞭子停了。鞭梢滞了半拍。再甩出去时，空了——没半点术法的重量。他低头把鞭子翻了个面，再抬左手——手背经脉还在跳，灵海里的气旋已经散了。」（公式④ 果先因后）
【Humanizer·禁止句式 6 种（命中即拆，不允许公式化对仗）】
1. 「虽然……但是……」—— 不用转折词，改用角色内心吐槽/前后动作反差写转折
2. 「不仅……而且……」—— 打散为两句，各写一个事实
3. 「第一/首先…… 第二/其次…… 第三……」列举腔 —— 改为散句叙述，两项/四项比三项自然
4. 破折号「——」—— 全章 0 次，用逗号或句号断句
5. 连续「了」字堆砌 —— 同一句只保留一个有力的"了"："他走了过去，拿了杯子喝了一口"→"他走过去，端起杯子灌了一口"
6. 排比三连句 / 三段式法则 —— 不要硬把想法分成三组，两项或一项更自然
【贴标签情绪禁词（用小动作+体感+反常行为代替，严禁直接结论式写）】
...
- 冰山理论：80% 心理藏在水下，只能 20% 外露成反常行为。
- 情绪分级：低强度过场情绪可一笔直写带动作（"他不耐烦地摆摆手"可以），关键剧情的重情绪禁止情绪词，必须按下方生理反应置换表给身体反应。
...
【冰山人物·角色情绪铁律使用上限（防同一情绪同角色反复套模板）】
同一角色、同一情绪类别（愤怒/恐惧/紧张/震惊/悲伤等），整章最多只允许 2 档身体反应（= 2 个动作组）；第 3 次再写同样情绪时，必须换一种"反常行为锚"（不说台词/不做同档位动作，直接干一件读者想不到的事），不能再继续堆同档位的身体反应。
例：林晚晴·恐惧档 1 = 指甲在纸面刮出细响；档 2 = 手指发白到指节咔咔响；若本章还需要第 3 次写林晚晴恐惧，绝对不许再写"手抖/呼吸乱/眼底水光"，必须改反常行为：「她抬起左手，把那份检测资料当众撕了一半，又塞回文件袋里。」（反常行为锚点，读者瞬间就能读到更重的情绪，不需要再堆同款小动作）。
...
相邻 3 句若同时满足以下三条件，必须至少合并相邻 2 句成 1 段：
  ...
  ② 场景一致（同地点/同镜头/同一时间片）
  ③ 动作链一致（同一件事的连续动作或对白）
连续 ≥ 4 句"一句话独立段"时，必须直接合并相邻 2–3 句，否则计为 AI 碎段。

## 动作链 KPI 闭环（落地铁律·防画面跳帧）
每个动作链（起手→过程→落点）必须有起点动作、过程动作、结果落点三段；
禁止起手没落地（例"他抬手"没了→必须补"他抬手挡开那根铁棍，指节震得发麻"）；
禁止画面没视角锚点（禁止写"铁剑落下。鲜血飞溅。"这种纯物体镜头，必须挂在视角人物能看见/能听见/能震到的点上）。

## Humanizer·叙事恶习 10 条（网文才会这样写，AI 绝对要戒掉）
1. **不替读者下结论**：读者能从行为推断的意图，叙述者绝对不直接说出（不说"他想讨好老板"→直接写"他抢在老板前面把烟灰缸端走了"）。
2. **不写分析报告**：正文中禁止出现"核心动机/信息边界/信息落差/利益最大化/底层逻辑/认知差/降维打击"这类分析术语。
3. **不堆叠从句**：一句话避免三个以上从句（含"因为/如果/虽然/当/只要/为了"），堆叠就拆成短句，短句天然网文味。
4. **不重复"他/她"开头**：连续两句以上"他……"就变一下——把动作/物象/声音描写放句首（"门被踹开的瞬间"→主动"门板砰地飞进来"；不说"他看见刀光闪"→直接写"一道刀光从右肩斜劈下来"）。
5. **不过度解释情绪**：情感时刻写完动作/画面就停，不要紧跟"他感到……"；让行动和对话自己说话（"他攥了一下午的欠条被他撕了。"就够了，别再跟"他感到一种前所未有的解脱"）。
6. **控制惊讶标记词密度**：仿佛/忽然/竟/竟然/猛地/猛然/不禁/宛如/蓦然/骤然 每 3000 字合计 ≤ 1 次（≈一章最多 1 次）；其余全部改动作/物象/感官。
【偷懒转场词（突然/忽然/骤然/陡然/蓦然/竟/竟然/猛然/不禁/宛如）— 出现即替换，不允许保留原词，必须用"物的中间过程 2 拍"替代】
替换公式：写"物出现变化之前，先写 1 拍中间态，再写变化后的结果"，不能直接"状态A+忽然+状态B"跳按钮。
• "忽然出现一层细细的蓝光" → 改成「蓝光先在鞋底冒了个针尖大的点。一点一点舔过水泥路。」
• "旧表忽然发热" → 改成「袖口先闷得发痒。接着左腕那块金属温起来——烫得像晒了一中午的扳手。」
• "蓝色界面骤然变红" → 改成「蓝色先深一层。然后右上角那行字开始渗血，整张界面红得快滴下来。」
• "他猛地向上一掰" → 改成「指腹先吃进金属缝里，皮破了。然后他把整个身子的重量压上去。」
• "陈烨脚步一顿"（其实是"忽然停住"的变体）→ 改成「鞋底在同一块地砖上蹭了三下，才停住。」
7. **同一意象域不连续渲染超过两轮**：比如"火在体内流动"的同一域（热/烧/烫/炙/焚）禁止连续写两轮以上，到第二轮就换动作或切镜头。
8. **禁止元叙事旁白**：不写"到这里算是钉死了""话说回来""这一幕恰好说明""这个伏笔后面会用到"，作者绝不跳出来说话。
9. **空洞强调词换精确描写**："非常/极其/十分/特别/无比/甚是/极为/颇"能用具体动作/数字/类比代替的全删（不说"他十分恐惧"→写"他的牙在打架，后背贴在墙上，冷汗浸透了三层布"；不说"非常冷"→写"哈出来的气白成了烟"）。
10. **不堆叠限定词**：避免"可能/大概/也许/某种程度上/一定程度上/基本上/几乎/差不多/几乎可以说"堆在一句里，要么肯定要么留白，不要半吊子。

## Humanizer·被动 → 主动（AI 最爱写"被"，网文一律改主动）
- 杯子被他捏碎了 → 他捏碎了杯子
- 消息被传到了城里 → 消息传到城里
- 门被风吹开了 → 风吹开了门
- 他被吓了一跳 → 他吓了一跳 / 他浑身一激灵
口诀：一句里出现"被"字就问自己"能不能把主语换成主动"，能就换。

## Humanizer·副词 → 动作/物象/感官（"X地"一律不用，用动作+声音+触感代替）
- 他冷冷地说 → 他说这句话时嘴角没翘，手指在桌沿轻轻敲了三下
- 她悄悄地走 → 她踮着脚尖，地板在脚下发出轻微的吱呀声
- 他快速地跑 → 他窜出去，鞋底在湿地上啪嗒啪嗒拍响
- 她慢慢地回头 → 她脖子像生了锈，半天才转回来
- 他死死地盯着 → 他盯得死死的，眼皮都没眨一下

## Humanizer·其他 AI 模式 6 条（网文不玩这些）
1. **系动词回避**：别为了假装高级不用简单的"是"。"这座城市被誉为东方明珠"→"这座城市是东方明珠"；"他被视为天才"→"他是个天才"。
2. **刻意换词别硬换**：不要为了避免重复去翻生僻同义词。宁可重复也不要别扭（"他"重复两三次没关系；"翌日/彼时/孰料"这种文言词跳出来比重复还难受）。
3. **虚假范围**："从 A 到 B""从 X 到 Y"的结构除非 A/B 真在有意义的尺度上（"从一楼到三楼"算，"从黎明到黄昏/从生到死"这种假范围一律删）。
4. **开头方式要杂**：连续三段开头方式相同（都是"他…"；都是"时间词+，"；都是环境描写）→ 必须把其中一段打乱，用动作/对白/物鸣叫开头。
5. **段落结尾别清一色收束**：一段结尾老是逗号/句号/工整对仗 → 用对白、用半截话、用物鸣、用动作切开，让节奏有呼吸感。
6. **句子长度差拉开**：连续三句长度差不多（都是 12–18 字）→ 中间插一句 4–6 字短句做重拍，或者插一句 26–32 字长句做拖音。

## Humanizer·反例 → 正例速查（写作时拿脑子过一遍）
| 反例（AI味） | 正例（网文味） |
|---|---|
...
## 推荐结构
动作起手用逗号串过程、一个句号收完整画面；对白独立成段或嵌动作同段；短句只用在转折和收尾做重拍，不连续超过 2 句。

## 写完自检（逐项检查，不达标必须改写，顺序不可跳）
- 段落是否大多 1–2 句成段？段内句号数是不是 ≤ 2？**段落字数≤15字的短段是不是严格只含 ≤1 个句号？**（硬卡4.1，>2 或 短段≥2句号 立即不合格）
- 叙述句里逗号长句（20-35字、串1-2个动作）是不是占了七成以上？有没有连续 3 句以上 <12 字句号短句（碎句）？有则合并成逗号长句。
- 短句是不是只用在转折/爆发/收尾做重拍？有没有连续超过 2 句短句？
- 句均字数是不是 18–28 字？段均句数是不是 ≤ 1.8？（硬卡4.3/4.4）
- 是否存在超过55字的叙述句？有则拆开。
- 对白段落约占25%-60%？每轮对白只推进一个信息点？
- 章尾是否留下明确钩子？
- 是否避免了总结性、评价性、升华的句子？
- 是否避免了大段心理独白和形容词堆砌？
- 描写是否服务情报、动作和悬念？
...
- **禁令0 5类修正式否定句是不是全章 0 次命中？**（不是A而是B/排比三连/自我修正/否定修辞/否定前置 全禁）
- **冰山人物：关键剧情的重情绪有没有偷懒用情绪词直写？**（重情绪必须改"反常行为+小动作+体感+微表情"或生理反应置换表；过场小情绪带动作一笔带过即可）
- **段落合并三同时：** 相邻 3 句同POV+同场景+同镜头是否合并至少 2 句？连续≥4句一句话独立段是否合并？
- **动作链 KPI：** 每个动作链有没有起点+过程+落点？有没有纯物体空镜头（没挂视角锚点）？
- **有没有上帝视角/剧透式叙述/上帝点评？**（"他不知道此时…""这个决定将改变命运""命运就是如此"等，有则删）
- **视角是否锁定？**同一段落内有没有跳到另一个人物心理？有则分场或删除。
- **伏笔是否隐性？**有没有明写"这是伏笔/此处埋线/后面会回收"？有则改成闲笔。
- **本章伏笔是否过载？**暗线超过2条？有则分散到其他章。
- **角色是否全知全能？**主角/反派有没有不合理地"料事如神"？有则让其失算一次。
- **摄像机词「看见/看着/听见/注意到/盯着/望向」合计是不是 ≤ 3 次？** 超则改"让事物自己动"（把观察对象放句首主动语态，直接描写对象本身的动作/声响/形态变化）。
- **Humanizer·快速检查 6 条（每条都要过脑子，缺哪条改哪条）**
  1. 连续三个句子长度差不多？ → 中间插一句 4–6 字短句做重拍，或插 26–32 字长句拖音
  2. 段落结尾清一色工整/句号收尾？ → 用对白、半截话、物鸣、动作切开，变换结尾方式
  3. 用了「此外/然而/因此/于是」等连接词？ → 删掉，让前后句的因果关系自然流出来
  4. 硬凑三段式「第一/第二/第三」列举？ → 改成两项或四项，改成散句
  5. 有金句体质的句子（听起来像名人名言/鸡汤）？ → 重写成动作/对白/事实
  6. 整章出现「被」「虽然…但是…」「不仅…而且…」「X地」副词「值得注意的是/综上所述」废话 16 词？ → 命中即改，不允许通过"""
...
- 硬卡4·【整章量化双轨自检（输出前必须逐条检查，任一不达标即判定为不合格章节必须重写）】
  4.1 段内句号数硬上限 ≡ 每段正常只含 1–2 个句号（=每段最多 2 句完整话）；整章中含 ≥ 3 个句号的段落数必须 = 0；**补充：凡段落字数（含标点）≤ 15 个字的短段，硬上限进一步降为 ≤ 1 个句号（短段里绝对不允许塞 2 句完整话）**；绝对不允许一段里堆 3–6 个小短句（漫画分镜脚本化是最浓AI味来源之一）
  4.2 段落结构 ≡ 叙述主力段 40–90 字（1–2 个逗号长句）；叙述短段（＜40字）占比 15%–25%（只用于转折/收尾）；对白段不限（一句一段是常态）；70% 的段落 ≤ 70 字（手机端三行以内）；不允许把一个自然动作链切成 4+ 句独立段
  4.3 句均字数 ≡ 叙述句平均 18–28 字（含标点），叙述句七成以上为逗号长句（整句 20–35 字、逗号串 1–2 个动作单元收在一个句号）；连续 ≥ 3 句 ＜ 12 字句号短句 = 漫画分镜碎句，必须合并成逗号长句
...
SCHEMA_SEED_VERSION = '2026-08-23.1'  # 递增：悬疑/言情文风包改逗号长句铁律，老库需重同步种子
