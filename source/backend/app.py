...
SEED_SKILL_PACKS = [
    ...
    {'name': '都市异能文风', 'description': '都市异能正文文风：现代口语+爽快节奏+异能展示与日常切换，番茄都市异能专用文风锚定',
     ...
     'prompts': json.dumps({
         'style_anchor': '你是都市异能文风锚定师。生成7维文风速查表：1)整体风格(现代/口语化/爽快) 2)长短句比例(短句45%-60%每句1-20字，中句20%-30%每句20-35字，长句10%-15%每句35-70字) 3)对话占比(单章25%-60%，现代口语+网络用语适度) 4)叙述规则(异能展示服务剧情，不堆设定) 5)信息推进(对话+动作双驱动，异能效果靠画面感) 6)章法(开头进事件，结尾留悬念或打脸) 7)禁用(古风词汇/文绉绉表达/大段心理独白)。',
    ...
    {'name': '悬疑文风', 'description': '悬疑正文文风：冷峻克制+信息差控制+伏笔精密埋设，盐选/起点悬疑专用文风锚定',
     ...
     'prompts': json.dumps({
         'style_anchor': '你是悬疑文风锚定师。生成7维文风速查表：1)整体风格(冷峻/克制/留白) 2)长短句比例(短句50%-60%制造紧张感，中句20%-30%交代线索，长句10%-15%只用于复杂推理) 3)对话占比(单章25%-60%，对话即信息博弈) 4)叙述规则(只给必要细节，留白制造悬念) 5)信息推进(每章释放1-2条线索，埋1-2个伏笔) 6)章法(开头进事件，结尾留钩子或反转) 7)禁用(情绪直述/全知视角剧透/大段心理分析)。',
    ...
    {'name': '历史文风', 'description': '历史正文文风：半文半白+权谋对话+史感节奏，起点历史/架空历史专用文风锚定',
     ...
     'prompts': json.dumps({
         'style_anchor': '你是历史文风锚定师。生成7维文风速查表：1)整体风格(沉稳/厚重/半文半白) 2)长短句比例(短句40%-50%用于决策/判断，中句20%-30%用于谋略交代，长句20%-30%用于场面/礼制说明) 3)对话占比(单章25%-60%，对话即权谋博弈) 4)叙述规则(礼制/称谓/官制准确，不堆史料) 5)信息推进(对话+事件双驱动) 6)章法(开头进事件，结尾留变数) 7)禁用(现代网络用语/白话直述/大段史料科普)。',
    ...
    {'name': '科幻文风', 'description': '科幻正文文风：冷静硬朗+技术细节+设定融入叙事，番茄科幻/硬科幻专用文风锚定',
     ...
     'prompts': json.dumps({
         'style_anchor': '你是科幻文风锚定师。生成7维文风速查表：1)整体风格(冷静/硬朗/技术感) 2)长短句比例(短句40%-55%用于反应/判断，中句25%-35%用于技术交代，长句10%-20%只用于复杂设定) 3)对话占比(单章25%-60%，对话推进剧情) 4)叙述规则(设定融入叙事不堆砌) 5)信息推进(对话+事件+技术展示三驱动) 6)章法(开头进事件，结尾留新发现/新危机) 7)禁用(古风词汇/抒情堆砌/大段技术科普)。',
    ...
    {'name': '言情文风', 'description': '言情正文文风：细腻克制+情感拉扯+对白潜台词，女频言情/甜宠专用文风锚定',
     ...
     'prompts': json.dumps({
         'style_anchor': '你是言情文风锚定师。生成7维文风速查表：1)整体风格(细腻/克制/留白) 2)长短句比例(短句45%-60%用于反应/对话，中句20%-30%用于情感描写，长句10%-20%只用于复杂心理) 3)对话占比(单章25%-60%，对话即情感博弈) 4)叙述规则(情感靠细节传递不直述) 5)信息推进(对话+微表情+小动作三驱动) 6)章法(开头进场景，结尾留情感钩子) 7)禁用(情绪直述/工业糖精/大段心理分析)。',
