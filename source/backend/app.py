...
SEED_SKILL_PACKS = [
    ...
    {'name': '玄幻小说文风', 'description': '冷静克制硬朗文风：短句密集+对话驱动+禁词管控+长短句比例控制，玄幻正文专用文风锚定', 'genre': 'fantasy', 'book_type': 'novel', 'icon':'⚔️',
     ...
     'prompts': json.dumps({
         'style_anchor': '你是玄幻文风锚定师。根据参考文本或偏好，生成7维文风速查表：1)整体风格(冷静/克制/硬朗) 2)长短句比例(短句40%-60%每句1-20字，中句20%-30%每句20-35字，长句10%-20%每句35-70字) 3)对话占比(单章25%-60%) 4)叙述规则(描写服务情报和动作，不堆氛围) 5)信息推进方式(对话式推理优先) 6)章法要求(开头进场景，结尾留钩子) 7)禁用倾向(大段心理独白、形容词堆砌、总结性句子)。每维一行，创作时照着写。',
         ...
         'rhythm_check': '你是句式节奏检查员。逐段检查以下维度：1)短句是否占40%-60%(1-20字)？2)中句是否占20%-30%(20-35字)？3)长句是否控制在10%-20%(35-70字)？4)是否存在超过70字的句子？5)对白段落是否约占25%-60%？6)每轮对白是否只推进一个信息点？7)章尾是否留下明确钩子？8)是否避免了总结性、评价性、升华的句子？9)是否避免了大段心理独白和形容词堆砌？10)描写是否服务情报、动作和悬念？11)有没有可以用动作代替的心理描写？12)内心独白是否控制在5%以内？输出每项的通过/未通过状态和具体位置。',
     }, ensure_ascii=False)},
    ...
    {'name': '都市异能文风', 'description': '都市异能正文文风：现代口语+爽快节奏+异能展示与日常切换，番茄都市异能专用文风锚定',
     ...
     'prompts': json.dumps({
         'style_anchor': '你是都市异能文风锚定师。生成7维文风速查表：1)整体风格(现代/口语化/爽快) 2)长短句比例(短句45%-60%每句1-20字，中句20%-30%每句20-35字，长句10%-15%每句35-70字) 3)对话占比(单章30%-60%，现代口语+网络用语适度) 4)叙述规则(异能展示服务剧情，不堆设定) 5)信息推进(对话+动作双驱动，异能效果靠画面感) 6)章法(开头进事件，结尾留悬念或打脸) 7)禁用(古风词汇/文绉绉表达/大段心理独白)。',
