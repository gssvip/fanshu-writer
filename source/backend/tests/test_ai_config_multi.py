...
def test_get_config_creates_default_if_empty(auth_client):
...
def test_list_configs_returns_at_least_one(auth_client):
    """GET /api/ai/configs 始终至少返回 1 条激活配置。"""
    # 先确保有一条
    auth_client.get("/api/ai/config")
...
def test_create_config_auto_activates(auth_client):
    """POST 新增配置后自动激活，旧的取消激活。"""
    # 先建一条默认
    auth_client.get("/api/ai/config")
    # 新增第二条
    resp = auth_client.post("/api/ai/configs", json={
...
def test_activate_switches_active(auth_client):
    ...
    auth_client.get("/api/ai/config")
    r2 = auth_client.post("/api/ai/configs", json={"name": "第二配置", "provider": "kimi"}).get_json()
    # 当前激活的是 r2（新增自动激活），切回第一条
    configs = auth_client.get("/api/ai/configs").get_json()["configs"]
    first_id = [c for c in configs if c["id"] != r2["id"]][0]["id"]
    resp = auth_client.put(f"/api/ai/configs/{first_id}/activate")
    assert resp.status_code == 200
    assert resp.get_json()["is_active"] is True

    # get_active 应返回切回去的那条
    active = auth_client.get("/api/ai/config").get_json()
    assert active["id"] == first_id


def test_delete_active_config_promotes_next(auth_client):
    """删除激活配置时自动激活剩下首条。"""
    auth_client.get("/api/ai/config")
    r2 = auth_client.post("/api/ai/configs", json={"name": "第二配置", "provider": "kimi"}).get_json()
    # r2 是激活的，删除它
    resp = auth_client.delete(f"/api/ai/configs/{r2['id']}")
    assert resp.status_code == 200
    # 删除后仍有一条激活
    active = auth_client.get("/api/ai/config").get_json()
...
def test_update_config_keeps_masked_key(auth_client):
    """PUT 更新时 api_key 为 '***' 不覆盖真实密钥。"""
    auth_client.get("/api/ai/config")
    auth_client.put("/api/ai/config", json={"api_key": "sk-real-secret"})
    # 用掩码更新其他字段，密钥应保留
    auth_client.put("/api/ai/config", json={"api_key": "***", "model": "new-model"})
    body = auth_client.get("/api/ai/config").get_json()
    assert body["model"] == "new-model"
    assert body["has_key"] is True


def test_create_same_provider_never_overwrites(auth_client):
    """同 provider 两次 POST → 两条独立配置，互不覆盖（核心回归：添加即覆盖 bug）。"""
    auth_client.get("/api/ai/config")
    r1 = auth_client.post("/api/ai/configs", json={
        "name": "智谱-主号", "provider": "zhipu",
        "base_url": "https://open.bigmodel.cn/api/paas/v4",
        "model": "glm-4-main", "api_key": "sk-main",
    }).get_json()
    r2 = auth_client.post("/api/ai/configs", json={
    ...
    configs = {c["id"]: c for c in auth_client.get("/api/ai/configs").get_json()["configs"]}
...
def test_update_config_by_id_isolated(auth_client):
    """PUT /configs/<id> 只改指定配置：不改激活状态、不串 key、不影响其他行。"""
    auth_client.get("/api/ai/config")
    auth_client.post("/api/ai/configs", json={
    ...
    resp = auth_client.put(f"/api/ai/configs/{r2['id']}", json={
...
def test_update_config_by_id_404(auth_client):
    """PUT 不存在的配置返回 404。"""
