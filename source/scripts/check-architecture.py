...
# 2026-08-18 重校准5（正文文风/对话硬约束补丁包）：
#   - 对白6式二次加固 + 强压迫场景压迫语气差硬卡 + 群像声纹差硬卡
#   - 段落合并硬执行（同POV×同场景×同镜头→必须合并）
#   - 动作闭环补挖矿/赶路/抄写/练功重复类
#   - 结尾真相直白点破禁（动态动作句收尾）
#   - 自检清单改为6条新必过
#   - 正文前置指令chapter_plot_iron追加5条前置提醒
#   - 事前禁令4条→7条（禁令5漫画分镜/禁令6压迫语气差/禁令7真相直白点破）
#   - DEAI同步：对白识别扩2+处理扩4+排比扩3类+词表2个必删句式+执行流程8→10条
#   → 净增 6922 - 6878 = 44 行（之前 6878 → 本轮 6922）
CHAT_COLLAB_BP_BASELINE = 6922
