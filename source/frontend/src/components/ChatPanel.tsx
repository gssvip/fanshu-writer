...
// 纯解析工具（SSE 流/章节号）：抽出为独立模块，供 vitest 单测覆盖（lib/parsing.ts）
import {
  parseSSE,
  formatChapterTitle,
  displayChapterNum,
...
// 卡片/消息渲染与工具栏：从本文件拆分到独立模块（见 ChatPanelCards.tsx / ChatPanelToolbars.tsx）
import { MessageBubble, RankScanCard, ProgressMapView, type CardApplyMode } from './ChatPanelCards';
import { SkillPackSelector, GeneralAssistantSelector } from './ChatPanelToolbars';
// 节点设计师独立组件：进度解析纯函数 + 进度条工具栏（自本文件拆出）
import { NodeDesignerProgress } from './ChatPanelNodeDesigner';
...
export default function ChatPanel() {
  ...
  const CHAPTER_DEFAULT_INPUT =
    '书接上文，读取剧情维度里的「本章剧情节点」，禁止超出本章剧情节点创作。语句自然顺畅，写事为主，景一笔带过，非必要不用比喻/拟人等修辞。';
  ...
  const inputPlaceholder = (() => {
    ...
    if (activeTab === 'chapter') return '书接上文，读取剧情维度里的「本章剧情节点」，禁止超出本章剧情节点创作。语句自然顺畅，写事为主，景一笔带过，非必要不用比喻/拟人等修辞。（可直接点「续写」，或追加要求，如：第3章 增加打斗细节）';
...
最后给出 SAVE_PLOT 卡片（nodes 字段含 characters/resources_gained/resources_used/total_resources_owned）便于采纳。`;
    ...
    if (_rtRole === 'roundtable') {
      ...
      try {
        ...
        if (e.name !== 'AbortError') {
          appendAiNotice('❌ 圆桌会议出错，点击继续或发一句"继续"即可从断点接着开会。');
          setStreamError('');
          removeEmptyAi();
        }
      } finally {
    ...
    try {
      ...
      await consumeSSE(res, ctrl, undefined, (kind: string, info: any) => {
        ...
        // 命中创作维度的 meta_json 不做前端气泡处理（顶部命中选择按钮已移除）
        // 通用聊天命中维度改由后端按对应维度的完整格式直接产出 CARD 卡片，可采纳落地。
      }, false, (freshSid: string) => {
                  ...
                  {selectedDim === 'general' ? (
                    <>
                      ...
                      {(sessionRoleMap[chatGeneralSessionId || '__general_pending_session__'] || 'default') === 'node_designer' && (
                        <NodeDesignerProgress messages={messages} streaming={streaming} onQuickContinue={handleQuickContinue} />
                      )}
                    </>
                  ) : (
                  ...
                  return (
                    <>
                      ...
                      >
                        ...
                        {showModelPicker && (() => {
                          // ① 组 = 各提供商配置（一行 = 一个提供商），组内 models = 用户在"我的"勾选的模型
                          ...
                          const _chooseModel = (cfg: AIConfig, model: string) => {
                            ...
                            api.selectAIConfigModel(cfg.id, model).then(updated => {
                              setAiConfigList(prev => prev.map(x => x.id === cfg.id ? updated : { ...x, is_active: false }));
                            }).catch(() => {});
                          };
                          return (
                            <div
                              ...
                              style={{
                                ...
                                minWidth: 290, maxHeight: 380, overflowY: 'auto', padding: 6,
                            ...
                            >
                              <div style={{ fontSize: 11, color: '#999', padding: '4px 8px 6px', borderBottom: '1px solid #f0f0f5', marginBottom: 4 }}>
                                已保存的提供商 · 点开选择模型，点选后智驾设定/正文/去AI/校审全部跟随
                              </div>
                              {_groups.map(g => {
                                const _open = _expandedKey === g.key;
                                const _curGroup = g.cfg.id === _chosen?.id;
                                return (
                                  <div key={g.key} style={{ marginBottom: 2 }}>
                                    ...
                                    <div
                                      ...
                                      style={{
                                        ...
                                        background: _curGroup ? '#f4f7ff' : 'transparent',
                                    ...
                                    {/* 组内：用户选定的模型（未选定的不放这里） */}
                                    {_open && (g.models.length === 0 ? (
                                      <div style={{ marginLeft: 18, padding: '6px 10px', fontSize: 12, color: '#999' }}>
                                        该提供商暂未选定模型，去"我的 · AI配置"勾选
                                      </div>
                                    ) : g.models.map(mid => {
                                      const active = _curGroup && _chosen?.model === mid;
                                      return (
                                        <div
                                          key={mid}
                                          onClick={() => _chooseModel(g.cfg, mid)}
                                          style={{
                                            marginLeft: 18, marginTop: 2, padding: '7px 10px', borderRadius: 8,
                                            cursor: g.has_key ? 'pointer' : 'not-allowed',
                                            display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 6,
                                            background: active ? '#eef3ff' : '#fafafc',
                                            border: active ? '1px solid #829cff' : '1px solid #ececf2',
                                            opacity: g.has_key ? 1 : 0.55,
                                          }}
                                        >
