...
export const CHAPTER_LANG_STYLES: Record<string, ChapterLangStyle> = {
  ...
  lengjun: { label: '冷峻', desc: '零度叙事，极度冷静客观，杜绝抒情与价值判断；短句为主，动词精准；以不动声色写残酷，情感内敛于事实之下。适合悬疑、谍战、现实悲剧与硬派推理。' },
  huali: { label: '华丽', desc: '辞藻丰赡，修辞密集，多用比喻通感与色彩词；意象层叠，追求感官冲击与画面张力；句式繁复绵密。适合玄幻史诗、宫廷权谋等需要宏大氛围的场景。' },
  kouyu: { label: '口语化', desc: '语言贴近日常口语，句式短、用词俗；可省主语、语序倒置、语气词丰富；叙述如说话，代入感强。适合都市生活、青春校园、第一人称与轻松吐槽向。' },
  dianying: { label: '电影感', desc: '借鉴镜头语言，远近中特写切换自如；用蒙太奇剪接跳过冗余过渡；光影天气暗示情绪，五感通感营造现场感。适合高画面需求、动作戏与影视化叙事。' },
  yishiliu: { label: '意识流', desc: '打破线性时间与逻辑因果，以人物意识流动推进；多用内心独白、记忆闪回、联想跳跃；语言可碎片化、语义朦胧。适合心理向、文艺实验与回忆梦境段落。' },
  baogao: { label: '报告体', desc: '模仿新闻报道或档案记录，语言客观简洁、信息密集；时间地点人物事件清晰，少修饰多白描；强调真实感与现场感。适合纪实向、悬疑案件与硬核设定段落。' },
  erciyuan: { label: '二次元', desc: '融合ACG语境，善用玩梗、吐槽与夸张颜艺描写；动作以"指令—反馈"式呈现，可加系统UI元素；节奏轻快，反差萌与中二感并存。适合轻小说、系统流与游戏异界。' },
  zhelisibian: { label: '哲理思辨', desc: '语言抽象凝练，常以议论介入叙事；多用设问、悖论与格言式句子；借人物之口探讨存在、命运与价值。适合问道向、严肃文学与成长思辨类。' },
  dianyaninglian: { label: '典雅凝练', desc: '用词考究文雅，句式工整有韵律；化用古语与典故而不晦涩；意境含蓄隽永，言简意丰。适合古言、文人雅士题材与需要文化底蕴的段落。' },
